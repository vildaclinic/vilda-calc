...
(function (global) {
  ...
  //
  // Bug 2 fix (2026-05): clearAllData() czyści formularz i localStorage, ale
  // NIE czyści sessionStorage-snapshotów używanych przez vilda_chrome (chip
  // pacjenta, chip użytkownika) ani nie odświeża mini-summary w decor-sidebar.
  // Bez tego po flow "guest → zaloguj się/korzystaj bez logowania → powrót"
  // chip i mini-summary pokazywały stare dane mimo czystego formularza.
  function resetAppSessionState(reason) {
    ...
    // Wyczyść sessionStorage-snapshoty trzymane przez vilda_chrome.js
    // (chip pacjenta + chip użytkownika). Bez tego chipy na innych stronach
    // pokazywałyby starego pacjenta / starego usera.
    try {
      if (global.sessionStorage) {
        global.sessionStorage.removeItem('vilda-chrome-patient-snapshot');
        global.sessionStorage.removeItem('vilda-chrome-user-snapshot');
      }
    } catch (e) {
      logError('resetAppSessionState (' + (reason || '') + ') sessionStorage cleanup', e);
    }

    // Dispatchujemy event żeby moduły UI (vilda_chrome chipy, custom-fixes
    // mini-summary, etc.) mogły się odświeżyć. Każdy moduł sam decyduje
    // czego potrzebuje — to jest broadcast cleanup.
    try {
      const evt = new global.CustomEvent('vilda:app-reset', { detail: { reason: reason || '' } });
      global.document.dispatchEvent(evt);
    } catch (_) { /* noop */ }

    // Synchronicznie wywołaj refresh chipów i mini-summary (jeśli są dostępne).
    // setTimeout 0 daje czas żeby clearAllData zakończył wszystkie kaskady.
    try {
      global.setTimeout(function () {
        try {
          if (typeof global.refreshUserChip === 'function') global.refreshUserChip();
          if (typeof global.refreshPatientChip === 'function') global.refreshPatientChip();
          if (typeof global.updateMiniSummary === 'function') global.updateMiniSummary();
        } catch (_) { /* noop */ }
      }, 50);
    } catch (_) { /* noop */ }
  }
  ...
  async function showLoginForUser(userId, options) {
    ...
    const sub = el('p', { class: 'vilda-auth-subtitle', text: 'Wpisz hasło, aby odblokować dane pacjentów.' });
    ...
    const screen = el('div', { class: 'vilda-auth-screen vilda-auth-login' }, [
      title, sub, banner, pw, errBox,
  ...
  async function boot() {
    ...
    try {
      ...
      getVault().onUnlock(function () {
        ...
        // Bug 2 fix (2026-05): jeśli przed logowaniem był tryb gościa, to
        // dane wpisane "jako gość" NIE są danymi tego, kto się właśnie
        // zalogował — trzeba je wyczyścić (formularz + sessionStorage chipy
        // + mini-summary). Vault potem potencjalnie load'uje lastPatient
        // tego usera, ale to osobny flow.
        const wasGuest = isGuestMode();
        if (wasGuest) {
          global[PWA_GUEST_FLAG] = false;
          persistGuestFlag(false);
          resetAppSessionState('on-unlock-from-guest');
        }
        showLogoutButton();
        startIdleWatch();
      });
