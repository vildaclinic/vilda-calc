...
(function () {
  ...
  function setupSgaBirthModule() {
    ...
    // P7 — Prefill z danych okołoporodowych pacjenta (payload.perinatal).
    // Zasada „wpisz raz, używaj wiele": gdy wczytany jest pacjent (event
    // vilda:patient-loaded lub window._vildaCurrentPatientId), uzupełniamy pola
    // SGA z karty pacjenta. Nazwy pól zgodne z kanonem/epikryzą. Nie nadpisujemy
    // pól, w których użytkownik już coś wpisał. Bez dispatchu change (żeby nie
    // odpalać walidacji w trakcie prefilla).
    function _sgaSetIfEmpty(elm, val) {
      if (!elm || val == null || val === '') return;
      if (String(elm.value).trim() !== '') return;
      elm.value = String(val);
    }
    function prefillFromPerinatal(patientId) {
      var V = window.VildaVault;
      if (!patientId || !V || typeof V.isUnlocked !== 'function' || !V.isUnlocked() ||
          typeof V.getPatient !== 'function') return;
      V.getPatient(patientId).then(function (pf) {
        var snap = (pf && Array.isArray(pf.snapshots) && pf.snapshots.length) ? pf.snapshots[0] : null;
        var payload = (snap && snap.payload) || {};
        var peri = payload.perinatal;
        if (!peri || typeof peri !== 'object') return;
        _sgaSetIfEmpty(gaWeeksSelect, peri.gestationalWeeks);
        _sgaSetIfEmpty(gaDaysSelect, peri.gestationalDays);
        _sgaSetIfEmpty(birthWeightInput, peri.birthWeightG);
        _sgaSetIfEmpty(birthLengthInput, peri.birthLengthCm);
        _sgaSetIfEmpty(birthHeadInput, peri.birthHeadCircCm);
        var sx = payload.user && payload.user.sex;
        if (sx && !sexInputs.some(function (i) { return i.checked; })) {
          var want = (sx === 'K' || sx === 'F' || sx === 'f' || sx === 'female') ? 'female' : 'male';
          var radio = sexInputs.find(function (i) { return i.value === want; });
          if (radio) radio.checked = true;
        }
      }).catch(function () {});
    }
    if (!window.__vildaSgaBirthPrefillBound) {
      window.__vildaSgaBirthPrefillBound = true;
      document.addEventListener('vilda:patient-loaded', function (e) {
        var pid = e && e.detail && e.detail.patientId;
        if (pid) prefillFromPerinatal(pid);
      });
      // P7b (2026-06-04): dokończenie triggerów — wejście na DocPro PO wczytaniu
      // pacjenta na innej stronie (index). Event patient-loaded nie strzela
      // ponownie w nowym dokumencie, a jednorazowa próba przy starcie przegrywała
      // wyścig z odblokowaniem vaulta (i z odtworzeniem globalnej przez
      // custom-fixes, który w DOM stoi dalej). Idempotentny retry czyta pid
      // BEZPOŚREDNIO z sessionStorage (vildaCurrentPatientId — utrwalany przez
      // custom-fixes od v=30) z fallbackiem na globalną i ponawia po odblokowaniu
      // (vilda:auth-hidden) oraz po dosynchronizowaniu cloud-only
      // (vilda:sync-status-changed; nadawca może użyć window lub document).
      // prefillFromPerinatal sam pilnuje guardów (unlock) i set-if-empty —
      // wielokrotne wywołania są bezpieczne i nic nie nadpisują.
      var _sgaPrefillRetry = function () {
        try {
          var pid = window._vildaCurrentPatientId || null;
          if (!pid && window.sessionStorage) pid = window.sessionStorage.getItem('vildaCurrentPatientId');
          if (pid) prefillFromPerinatal(pid);
        } catch (_) {}
      };
      document.addEventListener('vilda:auth-hidden', _sgaPrefillRetry);
      document.addEventListener('vilda:sync-status-changed', _sgaPrefillRetry);
      window.addEventListener('vilda:sync-status-changed', _sgaPrefillRetry);
      if (document.readyState !== 'loading') setTimeout(_sgaPrefillRetry, 0);
      else document.addEventListener('DOMContentLoaded', function () { setTimeout(_sgaPrefillRetry, 0); });
    }
    try { if (window._vildaCurrentPatientId) prefillFromPerinatal(window._vildaCurrentPatientId); } catch (_) {}
