...
function auditNavigationCoverage() {
  ...
  function attachServiceWorkerControllerChangeLifecycle(container) {
    ...
    container.addEventListener('controllerchange', () => {
      ...
      try {
        // Jeśli zainstalowany jest Turbo Drive, czyścimy jego cache i wyłączamy
        // tryb "drive", żeby `location.reload()` skutkował twardym reloadem
        // wszystkich assetów z nowego service-workera. Bez tego Turbo mogłoby
        // serwować stary HTML z własnego cache page-snapshots.
        if (typeof window !== 'undefined' && window.Turbo) {
          if (window.Turbo.cache && typeof window.Turbo.cache.clear === 'function') {
            try { window.Turbo.cache.clear(); } catch (_) {}
          }
          if (window.Turbo.session && typeof window.Turbo.session.drive === 'boolean') {
            try { window.Turbo.session.drive = false; } catch (_) {}
          }
        }
        window.location.reload();
      } catch (error) { state.lastError = error && error.message ? error.message : String(error || 'reload-error'); }
    });
  }
  ...
  async function setupSW() {
    ...
    // Service Worker wymaga zaufanego origina (https:, http://localhost,
    // http://127.0.0.1). Przy otwarciu pliku przez `file://` (lub w iframe
    // z innym schematem) próba rejestracji rzuca głośny warning. Wczesny
    // bail-out — bez błędu w konsoli.
    try {
      const proto = (typeof location !== 'undefined' && location.protocol) || '';
      if (proto !== 'https:' && proto !== 'http:') return null;
    } catch (_) { /* noop, idziemy dalej */ }
    const state = getSWClientLifecycleState();
