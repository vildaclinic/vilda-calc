...
(function (global) {
  ...
  // ── Efektywne okno bezczynności dla TTL sesji ──────────────────────────────
  // BUG-FIX (2026-06-04): persistSession() liczył expiresAtISO z idleTimeoutMs,
  // które w świeżym kontekście strony ZAWSZE było jeszcze domyślne (20 min) —
  // auth UI ustawia efektywne okno (cloud-select / „ufam urządzeniu") dopiero
  // w onUnlock→startIdleWatch, czyli PO persistSession(). Skutek: między
  // stronami obowiązywał sztywny sufit/podłoga 20 min niezależnie od ustawień.
  // Provider (ustawiany przez auth UI PRZED tryRestoreSession) zwraca okno
  // policzone z tych samych preferencji co timer in-page. Brak providera =
  // dotychczasowe zachowanie (fallback idleTimeoutMs). Bez wpływu na model
  // bezpieczeństwa klucza: klucz pozostaje wyłącznie w sessionStorage, a TTL
  // jest dokładnie oknem, na które użytkownik świadomie się zgodził w UI.
  let idleConfigProvider = null;
  // Dławienie odświeżania TTL przy aktywności (patrz refreshPersistedSession):
  // max 1 zapis/min — zero kosztu na hot-path zdarzeń wejściowych.
  const SESSION_REFRESH_THROTTLE_MS = 60 * 1000;
  let lastSessionRefreshAtMs = 0;
  ...
  // Efektywne okno bezczynności do TTL. Provider (auth UI) ma pierwszeństwo;
  // wynik jest walidowany (skończona liczba > 0), inaczej fallback do
  // idleTimeoutMs (dotychczasowe zachowanie). Defensywnie: błąd providera
  // NIGDY nie wydłuża sesji — fallback to konserwatywne 20 min default.
  function currentEffectiveIdleMs() {
    if (typeof idleConfigProvider === 'function') {
      try {
        const v = idleConfigProvider();
        if (typeof v === 'number' && isFinite(v) && v > 0) return v;
      } catch (_) { /* provider zepsuty → fallback niżej */ }
    }
    return idleTimeoutMs;
  }

  // Auth UI rejestruje tu effectiveIdleMs() PRZED bootem (vault.js ładuje się
  // przed vilda_auth_ui.js, a boot/tryRestoreSession rusza jeszcze później).
  function setIdleConfigProvider(fn) {
    if (typeof fn === 'function') idleConfigProvider = fn;
  }
  ...
  function persistSession() {
    try {
      ...
      const blob = {
        ...
        // TTL = EFEKTYWNE okno bezczynności (nie surowe idleTimeoutMs) —
        // patrz komentarz przy idleConfigProvider.
        expiresAtISO: new Date(Date.now() + currentEffectiveIdleMs()).toISOString()
      };
  ...
  // BUG-FIX (2026-06-04): „bezczynność" ma się liczyć od OSTATNIEJ AKTYWNOŚCI,
  // nie od logowania/nawigacji. Handler zdarzeń aktywności w auth UI woła tę
  // funkcję przy każdym geście; dławienie (60 s) trzyma koszt przy zerze.
  // Bez tego aktywnie pracujący użytkownik po >20 min na jednej stronie był
  // wylogowywany przy F5/przejściu na podstronę — mimo zerowej bezczynności.
  // Bezpieczeństwo: działa WYŁĄCZNIE przy odblokowanym vaulcie i tylko
  // przepisuje istniejący blob (nigdy nie tworzy sesji ani nie wydłuża jej
  // ponad okno wybrane przez użytkownika — expiresAt zawsze = teraz + okno).
  // force=true (zmiana ustawień okna w UI) pomija dławienie, żeby nowe,
  // np. KRÓTSZE okno obowiązywało w sessionStorage natychmiast.
  function refreshPersistedSession(force) {
    if (!isUnlocked()) return false;
    const now = Date.now();
    if (!force && (now - lastSessionRefreshAtMs) < SESSION_REFRESH_THROTTLE_MS) return false;
    lastSessionRefreshAtMs = now;
    persistSession();
    return true;
  }
   ...
   */
  async function updateSnapshotPayload(patientId, snapshotId, newPayload, options) {
    ...
    // P7-edit: preserveSavedAt → korekta IN-PLACE nie zmienia pozycji snapshotu
    // w osi savedAtISO (edycja/usuwanie wiersza pomiaru nie ma „przesuwać" wizyty
    // na koniec historii). Domyślnie (bez opcji) zachowanie jak dotąd: bump na teraz.
    const preserveSavedAt = !!(options && options.preserveSavedAt);
    const snapSavedAt = preserveSavedAt ? (existingSnap.savedAtISO || nowISO) : nowISO;
    ...
    await getAdapter().putSnapshotForUser(currentUserId, {
      snapshotId: snapshotId,
      patientId: patientId,
      savedAtISO: snapSavedAt,
      payloadCipher: newPayloadCipher
    });

    // Patient record — nagłówek + lastSavedAtISO. snapshotCount BEZ ZMIAN
    // (UPDATE, nie INSERT). createdAtISO zachowany. Przy preserveSavedAt nie
    // bumpujemy lastSavedAtISO (najnowszy snapshot się nie zmienił w osi).
    ...
    if (existingPatient) {
      await getAdapter().putPatientForUser(currentUserId, {
        ...
        lastSavedAtISO: preserveSavedAt ? (existingPatient.lastSavedAtISO || snapSavedAt) : nowISO,
  ...
  // ════════ P7-edit — EDYCJA/USUWANIE POJEDYNCZEGO WIERSZA POMIARU ════════
  // Punkty w Historii to WIERSZE tablicy pomiarowej (advanced→growthBasic) w
  // NAJNOWSZYM snapshocie, kotwiczone wiekiem — nie snapshoty. Te metody mutują
  // KONKRETNY wiersz (po uid; fallback klucz wiek|h|w) i zapisują przez
  // updateSnapshotPayload. Jeśli wiersz odpowiada bieżącemu pomiarowi (user.*),
  // synchronizujemy też user.*. uid jest leniwie backfillowany (migracja).
  function _measAgeMonths(row) {
    if (!row) return null;
    if (typeof row.ageMonths === 'number' && isFinite(row.ageMonths)) return Math.round(row.ageMonths);
    if (typeof row.ageYears === 'number' && isFinite(row.ageYears)) return Math.round(row.ageYears * 12);
    return null;
  }
  function _measKey(ageMonths, height, weight) {
    var am = (typeof ageMonths === 'number' && isFinite(ageMonths)) ? Math.round(ageMonths) : null;
    return am + '|' + ((typeof height === 'number' && isFinite(height)) ? height.toFixed(2) : '_')
              + '|' + ((typeof weight === 'number' && isFinite(weight)) ? weight.toFixed(2) : '_');
  }
  function _measKeyOf(row) { return _measKey(_measAgeMonths(row), row && row.height, row && row.weight); }
  function _measUserKey(user) {
    if (!user) return null;
    var am = null;
    if (typeof user.age === 'number' && isFinite(user.age)) {
      var extra = (typeof user.ageMonths === 'number' && isFinite(user.ageMonths)) ? user.ageMonths : 0;
      am = Math.round(user.age * 12 + extra);
    } else if (typeof user.ageMonths === 'number' && isFinite(user.ageMonths)) {
      am = Math.round(user.ageMonths);
    }
    if (am == null) return null;
    return _measKey(am, user.height, user.weight);
  }
  // Autorytatywna tablica = ta sama co czyta _extractMeasurementHistory: advanced→growthBasic.
  function _authoritativeMeasArray(payload) {
    var a = payload && payload.advanced && payload.advanced.data;
    if (a && Array.isArray(a.measurements) && a.measurements.length) return a.measurements;
    var g = payload && payload.growthBasic && payload.growthBasic.data;
    if (g && Array.isArray(g.measurements) && g.measurements.length) return g.measurements;
    if (a && Array.isArray(a.measurements)) return a.measurements;       // pusta advanced
    if (g && Array.isArray(g.measurements)) return g.measurements;       // pusta growthBasic
    return null;
  }
  function _ensureMeasRowUids(arr) {
    if (!Array.isArray(arr)) return;
    for (var i = 0; i < arr.length; i += 1) {
      var r = arr[i];
      if (r && typeof r === 'object' && !(typeof r.uid === 'string' && r.uid)) {
        r.uid = 'm-' + generateUuid();
      }
    }
  }
  function _findMeasRowIndex(arr, ref) {
    if (!Array.isArray(arr) || !ref) return -1;
    if (typeof ref.uid === 'string' && ref.uid) {
      for (var i = 0; i < arr.length; i += 1) if (arr[i] && arr[i].uid === ref.uid) return i;
    }
    if (typeof ref.key === 'string' && ref.key) {
      for (var j = 0; j < arr.length; j += 1) if (arr[j] && _measKeyOf(arr[j]) === ref.key) return j;
    }
    return -1;
  }
  function _applyAgeToUser(user, ageMonths) {
    if (ageMonths == null) return;
    user.age = Math.floor(ageMonths / 12);
    user.ageMonths = ageMonths % 12;
  }

  /**
   * P7-edit — aktualizuje jeden wiersz pomiaru (po uid/kluczu) w najnowszym snapshocie.
   * @param {string} patientId
   * @param {{uid?:string, key?:string}} ref — tożsamość wiersza
   * @param {{ageMonths?:number, height?:number, weight?:number, boneAgeYears?:number}} values
   */
  async function updateMeasurementRow(patientId, ref, values) {
    if (!isUnlocked()) throw new Error('Zaloguj się, by edytować pomiar.');
    if (typeof patientId !== 'string' || !patientId) throw new Error('updateMeasurementRow: brak patientId.');
    ref = ref || {}; values = values || {};
    var pf = await getPatient(patientId);
    if (!pf || !Array.isArray(pf.snapshots) || !pf.snapshots.length) throw new Error('updateMeasurementRow: brak pacjenta.');
    var latest = pf.snapshots[0];
    var payload = latest.payload || {};
    var arr = _authoritativeMeasArray(payload);
    var userKey = _measUserKey(payload.user);
    var nAge = (typeof values.ageMonths === 'number' && isFinite(values.ageMonths)) ? Math.round(values.ageMonths) : null;
    var nH = (typeof values.height === 'number' && isFinite(values.height)) ? values.height : null;
    var nW = (typeof values.weight === 'number' && isFinite(values.weight)) ? values.weight : null;
    var wasCurrent = false;

    if (arr) {
      _ensureMeasRowUids(arr);
      var idx = _findMeasRowIndex(arr, ref);
      if (idx < 0) throw new Error('updateMeasurementRow: nie znaleziono wiersza pomiaru.');
      var row = arr[idx];
      if (userKey && _measKeyOf(row) === userKey) wasCurrent = true;
      if (nAge != null) { row.ageMonths = nAge; row.ageYears = nAge / 12; }
      if (nH != null) row.height = nH;
      if (nW != null) row.weight = nW;
      if (typeof values.boneAgeYears === 'number' && isFinite(values.boneAgeYears)) row.boneAgeYears = values.boneAgeYears;
    } else {
      // Brak tablicy — edycja dozwolona tylko dla bieżącego pomiaru (user.*).
      if (!(userKey && ref.key === userKey)) throw new Error('updateMeasurementRow: brak tablicy pomiarów.');
      wasCurrent = true;
    }

    if (wasCurrent) {
      payload.user = payload.user || {};
      if (nH != null) payload.user.height = nH;
      if (nW != null) payload.user.weight = nW;
      if (nAge != null) _applyAgeToUser(payload.user, nAge);
    }
    await updateSnapshotPayload(patientId, latest.snapshotId, payload, { preserveSavedAt: true });
    return { patientId: patientId, updated: true, wasCurrent: wasCurrent };
  }

  /**
   * P7-edit — usuwa jeden wiersz pomiaru (po uid/kluczu) z najnowszego snapshotu.
   * NIE usuwa snapshotu. Jeśli usuwany był bieżącym, user.* przesuwa się na
   * najnowszy pozostały wiersz (max ageMonths).
   */
  async function deleteMeasurementRow(patientId, ref) {
    if (!isUnlocked()) throw new Error('Zaloguj się, by usunąć pomiar.');
    if (typeof patientId !== 'string' || !patientId) throw new Error('deleteMeasurementRow: brak patientId.');
    ref = ref || {};
    var pf = await getPatient(patientId);
    if (!pf || !Array.isArray(pf.snapshots) || !pf.snapshots.length) throw new Error('deleteMeasurementRow: brak pacjenta.');
    var latest = pf.snapshots[0];
    var payload = latest.payload || {};
    var arr = _authoritativeMeasArray(payload);
    if (!arr) throw new Error('deleteMeasurementRow: brak tablicy pomiarów.');
    _ensureMeasRowUids(arr);
    var idx = _findMeasRowIndex(arr, ref);
    if (idx < 0) throw new Error('deleteMeasurementRow: nie znaleziono wiersza pomiaru.');
    var userKey = _measUserKey(payload.user);
    var wasCurrent = userKey && _measKeyOf(arr[idx]) === userKey;
    arr.splice(idx, 1);
    if (wasCurrent && arr.length) {
      var newest = arr.slice().sort(function (a, b) { return (_measAgeMonths(b) || 0) - (_measAgeMonths(a) || 0); })[0];
      if (newest) {
        payload.user = payload.user || {};
        if (typeof newest.height === 'number') payload.user.height = newest.height;
        if (typeof newest.weight === 'number') payload.user.weight = newest.weight;
        _applyAgeToUser(payload.user, _measAgeMonths(newest));
      }
    }
    await updateSnapshotPayload(patientId, latest.snapshotId, payload, { preserveSavedAt: true });
    return { patientId: patientId, deleted: true, wasCurrent: wasCurrent, remaining: arr.length };
  }
  ...
  function _extractMeasurementHistory(snapshot) {
    ...
    for (var i = 0; i < sourceRows.length; i += 1) {
      ...
      // P7-edit: przenieś trwały uid wiersza (jeśli jest) — pozwala UI edytować/usuwać
      // KONKRETNY wiersz, niezależnie od zmiany wartości (klucz wiek|h|w to fallback).
  ...
  async function listPatientTimelineEvents(patientId) {
    ...
    if (patient && Array.isArray(patient.snapshots)) {
      ...
      // Rebuild measurements ASC po wieku (po dedup'ie + ewentualnym safety-net fallback).
      ...
      for (var mi = 0; mi < measurements.length; mi += 1) {
        ...
        events.push({
          ...
          // P7-edit: tożsamość WIERSZA (nie snapshotu) do edycji/usuwania pojedynczego
          // punktu. uid = trwały identyfikator (gdy jest); rowKey = fallback wiek|h|w.
          uid: (typeof curr.uid === 'string' && curr.uid) ? curr.uid : null,
          rowKey: measureKey
        });
      }
    }
  ...
  async function mergeSyncPayload(rawData, options) {
    ...
    try {
      ...
      if (_notesSupported) {
        ...
        // C2 ROLLBACK: sequential encrypt+put (stary, sprawdzony wzorzec).
    ...
    try {
      ...
      if (_pnSupported) {
        ...
        // C2 ROLLBACK: sequential encrypt+put.
        // Scal żywe notatki przychodzące.
  ...
  const api = {
    ...
    updateMeasurementRow: updateMeasurementRow,
    ...
    // TTL sesji wg efektywnego okna bezczynności (fix 2026-06-04):
    setIdleConfigProvider: setIdleConfigProvider,
    refreshPersistedSession: refreshPersistedSession,
    onUnlock: onUnlock,
    onLock: onLock,
