(function () {
  ...
  // Idempotency guard: globalne listenery na document/window instalujemy
  // dokładnie RAZ, niezależnie od ile razy `vildaOnReady` re-firuje
  // initPersistence przy turbo:load. Bez tej flagi listenery akumulowały się
  // przy każdej nawigacji wracającej na docpro.html.
  let globalListenersInstalled = false;
  // Wszystkie setTimeouty z restoreState trzymamy w secie, żeby skasować je
  // gdy user opuszcza stronę (turbo:before-cache) — inaczej fired'y w
  // tle robiłyby restoreUi/applyControls przeciwko żywemu DOM-owi innej strony.
  const pendingRestoreTimers = new Set();

  function logDocproPersistError(message, error, meta) {
    try {
      const logger = window.VildaLogger || window.vildaLogger || null;
      if (logger && typeof logger.error === 'function') {
        logger.error('docpro-state-persist', message || 'Błąd persistence DocPro', error || null, meta || null);
      }
    } catch (loggingError) {
      if (typeof window !== 'undefined' && window.__VILDA_DEBUG && window.console && typeof window.console.warn === 'function') {
        window.console.warn('[VildaLogger][docpro-state-persist] Nie udało się zapisać logu diagnostycznego', loggingError);
      }
    }
  }

  function logDocproPersistWarn(message, error, meta) {
    try {
      const logger = window.VildaLogger || window.vildaLogger || null;
      if (logger && typeof logger.warn === 'function') {
        logger.warn('docpro-state-persist', message || 'Ostrzeżenie persistence DocPro', error || null, meta || null);
      }
    } catch (loggingError) {
      if (typeof window !== 'undefined' && window.__VILDA_DEBUG && window.console && typeof window.console.warn === 'function') {
        window.console.warn('[VildaLogger][docpro-state-persist] Nie udało się zapisać logu diagnostycznego', loggingError);
  ...
  function isSaveSuppressed() {
    try {
      ...
      logDocproPersistWarn('Nie udało się sprawdzić blokady zapisu persistence DocPro', error);
      return false;
    }
  }
  ...
  function getPersistence() {
    ...
    } catch (error) { logDocproPersistWarn('Zignorowany błąd pomocniczy w persistence DocPro', error); }
    return null;
  }

  if (!getPersistence()) return;

  function loadState() {
    const persistence = getPersistence();
    if (persistence && typeof persistence.readDocproUi === 'function') {
      return persistence.readDocproUi();
    }
    return null;
  }
  ...
  function saveRaw(state) {
    ...
    const persistence = getPersistence();
    if (persistence && typeof persistence.writeDocproUi === 'function') {
      persistence.writeDocproUi(state);
      return;
    }
    /* Adapter niedostępny — brak-op. */
  }
  ...
  function clearState() {
    ...
    try {
      const persistence = getPersistence();
      if (persistence && typeof persistence.clearDocproUi === 'function') {
        persistence.clearDocproUi({ includeLegacy: true, markClear: true, durationMs: 1500 });
        return;
      }
    } catch (error) { logDocproPersistWarn('Zignorowany błąd pomocniczy w persistence DocPro', error); }
    try {
  ...
  function clearLegacyState() {
    try {
      const persistence = getPersistence();
      if (persistence && typeof persistence.clearLegacyDocproState === 'function') {
        persistence.clearLegacyDocproState();
        return;
      }
    } catch (error) { logDocproPersistWarn('Zignorowany błąd pomocniczy w persistence DocPro', error); }
    /* Adapter jest wymagany dla legacy clean-up; brak adaptera oznacza brak-op. */
  }
  ...
  function safeClick(el) {
    ...
    try {
      ...
      logDocproPersistWarn('Nie udało się kliknąć elementu przy odtwarzaniu UI DocPro', error, { id: el && el.id });
      try {
        el.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window }));
      } catch (fallbackError) { logDocproPersistWarn('Nie udało się wysłać zastępczego kliknięcia przy odtwarzaniu UI DocPro', fallbackError, { id: el && el.id }); }
    }
  }
  ...
  function startMutationObserver(state) {
    ...
    if (mutationObserverTimer != null) {
      try { window.clearTimeout(mutationObserverTimer); } catch (_) {}
    ...
    try {
      ...
      // Trackowany timer (przez własną zmienną) zamiast anonimowego setTimeout,
      // żeby przypadkiem nie disconnectował NOWEGO observera utworzonego
      // przez kolejną nawigację w ciągu 4s. Plus auto-clean na turbo:before-cache.
      mutationObserverTimer = window.setTimeout(() => {
        if (mutationObserver) {
          try { mutationObserver.disconnect(); } catch (_) {}
      ...
      logDocproPersistWarn('Nie udało się uruchomić MutationObserver dla persistence DocPro', error);
      mutationObserver = null;
    }
  }

  // Tracked setTimeout — id wpada do `pendingRestoreTimers`, callback usuwa
  // go z setu po wykonaniu, a `teardownPersistence()` kasuje wszystkie
  // pending na turbo:before-cache. Bez tego restore-timery odpalały się
  // długo PO opuszczeniu docpro i robiły restoreUi przeciwko DOM-owi
  // sąsiedniej strony.
  function trackedRestoreTimeout(fn, ms) {
    const id = window.setTimeout(() => {
      pendingRestoreTimers.delete(id);
      try { fn(); } catch (e) {
        logDocproPersistWarn('Błąd w tracked restore timeout', e);
      }
    }, ms);
    pendingRestoreTimers.add(id);
    return id;
  }

  function clearAllPendingRestoreTimers() {
    pendingRestoreTimers.forEach((id) => {
      try { window.clearTimeout(id); } catch (_) {}
    });
    pendingRestoreTimers.clear();
  }

  // Wywoływany przez turbo:before-cache — sprząta WSZYSTKIE per-page
  // resources. Listenery globalne (document/window) zostają — ich i tak nie
  // dotykamy, są one-shot zainstalowane.
  function teardownPersistence() {
    ...
    if (saveTimer != null) {
      try { window.clearTimeout(saveTimer); } catch (_) {}
      saveTimer = null;
    }
    clearAllPendingRestoreTimers();
    restoring = false;
  }

  function isOnDocproPage() {
    try {
      const path = (typeof location !== 'undefined' && location.pathname) || '';
      const file = path.substring(path.lastIndexOf('/') + 1);
      // Heurystyka: docpro.html lub element z #isDoctor (sygnatura strony)
      if (file === 'docpro.html') return true;
      if (typeof document !== 'undefined' && document.getElementById('isDoctor')) return true;
      return false;
    } catch (_) {
      return false;
    }
  ...
  // Globalne listenery na document/window — instalujemy DOKŁADNIE RAZ.
  // Bez tej separacji każdy `vildaOnReady('docpro-state-persist:init', ...)`
  // re-fire (czyli każda Turbo-nawigacja wracająca na docpro) dodawał kolejne
  // 4 listenery na `document` + 1 na `window` + 1 visibilitychange.
  // Po N nawigacjach każdy keystroke triggerował N save'ów.
  function installGlobalListeners() {
    if (globalListenersInstalled) return;
    globalListenersInstalled = true;

    document.addEventListener('input', (ev) => {
      if (!isOnDocproPage()) return;
    ...
    // pagehide odpala się tylko przy FULL page unload (close tab, hard reload).
    // Dla nawigacji Turbo używamy `turbo:before-cache` (poniżej).
    ...
    // Turbo lifecycle: przed cache'owaniem strony (czyli zaraz przed
    // opuszczeniem docpro) zapisz finalny state i sprzątaj per-page resources.
    document.addEventListener('turbo:before-cache', () => {
      if (isOnDocproPage() && !restoring && !isSaveSuppressed()) {
        try { saveRaw(buildState()); } catch (_) {}
      }
      teardownPersistence();
    });
  }
  ...
  function initPersistence() {
    // installGlobalListeners jest idempotentne — przy pierwszym wywołaniu
    // rejestruje listenery na document/window, kolejne re-fire'y to no-op.
    installGlobalListeners();

    // Per-page restore — tylko gdy faktycznie jesteśmy na docpro.
    if (!isOnDocproPage()) {
      // Defensywnie: gdyby cokolwiek wisiało z poprzedniej wizyty na docpro
      // (turbo:before-cache powinien już to wyczyścić, ale zostawiamy belt
      // & suspenders).
      teardownPersistence();
      return;
    }
  ...
  if (typeof window.vildaOnReady === 'function') {
    window.vildaOnReady('docpro-state-persist:init', initPersistence);
