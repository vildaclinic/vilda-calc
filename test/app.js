  ...
  */
  ...
  function __renderPrevSummary(data){
    ...
    if (!isAdult) {
      ...
      // Klasyfikacja wagi: poniżej 3. lub powyżej 97. centyla – czerwony alert
      if (weightPerc != null && (weightPerc < 3 || weightPerc > 97)) {
...
(function() {
  ...
  function updateDietRecommendationsVisibility() {
    ...
    // Dla dzieci (<18 lat) przycisk pojawia się tylko w trybie profesjonalnym
    if (!proMode && age < 18) return;
    ...
    if (age >= 18) {
      ...
      // Dla dzieci oblicz wskaźnik Cole’a: BMI / medianBMI * 100
      if (typeof getLMS === 'function') {
        const months = Math.round(age * 12);
        const lms = getLMS(document.getElementById('sex')?.value || 'M', months);
        if (lms && Array.isArray(lms) && lms.length > 1 && lms[1] > 0) {
          const cole = (bmi / lms[1]) * 100;
          if (cole > 110) show = true;
        }
      }
    }
  ...
  function generateDietRecommendations() {
    ...
    if (age >= 18) {
      ...
      if (kgToUpper > 0) {
        lines.push(`Twoja aktualna waga to ${weight.toFixed(1)} kg. Aby osiągnąć górną granicę normy BMI (24,9), musisz schudnąć ok. ${kgToUpper.toFixed(1)} kg.`);
        lines.push(`Do osiągnięcia wagi odpowiadającej 50. centylowi BMI (ok. ${targetMedianWeight.toFixed(1)} kg) brakuje około ${kgToMedian.toFixed(1)} kg.`);
      } else {
        lines.push(`Twoja masa ciała mieści się w granicach normy BMI. Jeżeli pragniesz dążyć do wartości odpowiadającej 50. centylowi BMI (22,0), docelowa waga wynosi ${targetMedianWeight.toFixed(1)} kg.`);
      }
      if (chosenDiet) {
        lines.push(`Wybrana dieta: ${chosenDiet.name}. Deficyt energetyczny wynosi około ${dailyDeficit} kcal/dzień, co przekłada się na utratę ok. ${weeklyLoss.toFixed(1)} kg tygodniowo.`);
      ...
      // Określ, czy dziecko ma nadwagę/otyłość na podstawie percentyla Cole’a (≥85. centyl)
      let childOverweight = false;
      let childObese = false;
      if (colePercent && !isNaN(colePercent)) {
        childOverweight = colePercent >= CHILD_THRESH_OLAF.NORMAL_HI;
        // Jeśli percentyl BMI ≥ OBESE (97. centyl), klasyfikujemy jako otyłość
        childObese = colePercent >= CHILD_THRESH_OLAF.OBESE;
      }
      // Wylicz docelową wagę odpowiadającą 110% mediany (górna granica normy) i 100% mediany (50. centyl)
      let targetWeightNorm = null;
      let targetWeightMedian = null;
      if (colePercent && colePercent > 0) {
        targetWeightNorm = currentWeight * (110 / colePercent);
        targetWeightMedian = currentWeight * (100 / colePercent);
      } else if (medianBMI) {
        const base = medianBMI * Math.pow(height / 100, 2);
        targetWeightNorm = base * 1.10;
        targetWeightMedian = base;
      }
      if (targetWeightNorm !== null && !isNaN(targetWeightNorm)) {
        ...
        if (toChild) {
          lines.push(
            `Twoja obecna waga to ${currentWeight.toFixed(1)} kg. Aby osiągnąć wagę w górnej granicy normy dla Twojego wieku i wzrostu, powinna wynosić ok. ${targetWeightNorm.toFixed(1)} kg, czyli trzeba zredukować około ${diffNorm.toFixed(1)} kg. Waga przeciętnego dziecka w Twoim wieku to ok. ${targetWeightMedian.toFixed(1)} kg, co oznacza, że do średniej brakuje około ${diffMedian.toFixed(1)} kg.`
          );
        } else {
          lines.push(
            `Waga dziecka: ${currentWeight.toFixed(1)} kg. Aby masa ciała znalazła się w górnej granicy normy dla wieku, powinna wynosić ok. ${targetWeightNorm.toFixed(1)} kg (czyli około ${diffNorm.toFixed(1)} kg mniej). Przeciętna waga rówieśnika to ok. ${targetWeightMedian.toFixed(1)} kg, zatem do 50. centyla brakuje około ${diffMedian.toFixed(1)} kg.`
          );
        }
      } else {
      ...
      if (growthEndedEffective) {
        ...
        if (toChild) {
          ...
          let mphInfo = '';
          if (agd && agd.targetHeight && !isNaN(agd.targetHeight)) {
            mphInfo = ` (Twój docelowy wzrost na podstawie wzrostu rodziców to ok. ${agd.targetHeight.toFixed(1)}\u00A0cm ±8,5\u00A0cm, więc ta prognoza jest tylko orientacyjna)`;
          }
          ...
          lines.push(
            `Pamiętaj, że ciągle rośniesz – prognozujemy, że możesz jeszcze urosnąć ok. ${remainingCm.toFixed(1)}\u00A0cm${mphInfo}. Każdy dodatkowy centymetr wzrostu obniży Twoje BMI. Staraj się, by w tym czasie Twoja masa ciała rosła minimalnie – dzięki temu „wyrośniesz” z ${growFrom}.`
          );
        } else {
          // Objaśnienie dla rodzica zawierające MPH i tolerancję ±8,5 cm.
          let mphInfo = '';
          if (agd && agd.targetHeight && !isNaN(agd.targetHeight)) {
            mphInfo = ` (docelowy wzrost na podstawie wzrostu rodziców to ok. ${agd.targetHeight.toFixed(1)}\u00A0cm ±8,5\u00A0cm, więc ta prognoza jest przybliżona)`;
          }
          const growFrom = childObese ? 'otyłości' : 'nadwagi';
          ...
          lines.push(
            `Dziecko wciąż rośnie – prognozujemy, że może jeszcze urosnąć ok. ${remainingCm.toFixed(1)}\u00A0cm${mphInfo}. Jeżeli masa ciała dziecka będzie rosła minimalnie, BMI będzie spadać w miarę wzrostu – wówczas dziecko „wyrośnie” z ${growFrom}.`
          );
        }
      } else {
      ...
      if (chosenDiet) {
        ...
        if (toChild) {
          ...
          let msg = `Optymalna dieta dostosowana do Twojego wieku, wzrostu, masy ciała i poziomu aktywności dostarcza około ${kcalPart} dziennie. Deficyt kaloryczny przy wybraniu tej diety wynosi około ${deficit}\u00A0kcal, co przekłada się na utratę ok. ${weekly.toFixed(1)}\u00A0kg na tydzień.`;
          // Dodaj objaśnienie w nawiasie dotyczące BMR, PAL i procentu redukcji
          if (energyRequirement > 0 && percentDef !== null) {
            msg += ` (Twoje szacowane zapotrzebowanie energetyczne (PPM × PAL) wynosi około ${energyReqRounded}\u00A0kcal dziennie przy ${palDescription}. Aby dieta była bezpieczna, odjęliśmy około ${percentDef}% energii, co odpowiada deficytowi ${deficit}\u00A0kcal). `;
          } else {
            msg += ' ';
          }
          if (selectedStrategy === 'reduction') {
          ...
          let msg = `Optymalna dieta dla dziecka, uwzględniająca wiek, wzrost, masę ciała i poziom aktywności, dostarcza około ${kcalPartChild} dziennie. Deficyt kaloryczny przy wyborze tej diety wynosi około ${deficit}\u00A0kcal, co przekłada się na utratę ok. ${weekly.toFixed(1)}\u00A0kg tygodniowo.`;
          // Objaśnienie w nawiasie dotyczące BMR dziecka i procentowej redukcji
          if (energyRequirement > 0 && percentDef !== null) {
            msg += ` (Szacowane zapotrzebowanie energetyczne dziecka (PPM × PAL) wynosi około ${energyReqRounded}\u00A0kcal dziennie przy ${palDescription}. Aby dieta była bezpieczna dla dziecka, odjęto około ${percentDef}% energii, co odpowiada deficytowi ${deficit}\u00A0kcal). `;
          } else {
            msg += ' ';
          }
          if (selectedStrategy === 'reduction') {
      ...
      if (childOverweight && hydrationEnabled) {
        ...
        if (toChild) {
          hydrationMsg =
            `Pamiętaj o odpowiednim nawodnieniu – Światowa Organizacja Zdrowia zaleca picie około 30 ml wody na każdy kilogram masy ciała (przy Twojej masie to ok. ${weightLiters.toFixed(1)} l dziennie). ` +
            `Zgodnie z polskimi normami żywienia dzieci w Twoim wieku i płci powinny wypijać około ${normativeLiters.toFixed(2)} l wody dziennie. ` +
            `Przy nadwadze lub otyłości staraj się wypijać co najmniej ${recLiters.toFixed(1)} l niesłodzonych płynów każdego dnia.`;
        } else {
          hydrationMsg =
            `Rodzice powinni zadbać o odpowiednie nawodnienie dziecka. Światowa Organizacja Zdrowia zaleca picie około 30 ml wody na każdy kilogram masy ciała – przy obecnej masie to ok. ${weightLiters.toFixed(1)} l dziennie. ` +
            `Polskie normy żywienia dla dzieci w wieku ${Math.floor(age)} lat i płci ${sex === 'K' ? 'żeńskiej' : 'męskiej'} zalecają około ${normativeLiters.toFixed(2)} l wody dziennie. ` +
            `W przypadku nadwagi lub otyłości dziecko powinno wypijać co najmniej ${recLiters.toFixed(1)} l niesłodzonych napojów każdego dnia.`;
        }
      ...
      if (childOverweight && journeyEnabled) {
        ...
        if (kgToLoseJourney > 0 && weeklyLoss > 0) {
          ...
          const monthsJourney = (weeksJourney / 4.345).toFixed(1);
...
function updateProfessionalSummaryCard(_retry) {
  ...
  function determineSummaryColor(line) {
    try {
      ...
      // Colour lines with explicit percentiles (weight, height, blood pressure, head/chest circumference).
      // Extreme percentiles (<=3 or >=97) → danger; near extremes (<=5 or >=95) → warning.
  ...
  function buildColumnRows(items) {
    ...
    items.forEach(txt => {
      ...
      // Apply colour coding: determine the colour based on the content of the line.
...
function adjustTestButtonWidths() {
  ...
  const ids = ['toggleGhTests', 'toggleOgttTests', 'toggleActhTests', 'toggleEndoTests', 'toggleIgfTests', 'toggleAbxTherapy', 'toggleZscore', 'toggleBisphos'];
  const isTwoColumn = window.innerWidth >= 700;
...
function formatCentileLabel(p) {
  ...
  // Odznacz wersje HTML entity jeśli się pojawiły
  const txt = raw.replace(/&lt;/g, '<').replace(/&gt;/g, '>');
  if (txt.startsWith('<')) {
    // poniżej 1 centyla
    // ucinamy znak „<” i spację (jeśli jest) i budujemy opis
  ...
  // w pozostałych przypadkach zwracamy „N centyl” – bez odmiany liczby
  return `${txt} centyl`;
}
...
function generateMetabolicSummary() {
  ...
  if (ageYears > 0 && weightVal > 0 && heightVal > 0) {
    ...
    if (coleVal !== null && coleVal !== undefined && !isNaN(coleVal)) {
      lines.push(`Wskaźnik Cole’a: ${coleVal.toFixed(1)}%`);
    }
    ...
    try {
      ...
      if (waistVal && !isNaN(waistVal) && waistVal > 0) {
        let line = `Obwód talii: ${waistVal.toFixed(1)} cm`;
      ...
      if (hipVal && !isNaN(hipVal) && hipVal > 0) {
        let line = `Obwód bioder: ${hipVal.toFixed(1)} cm`;
      ...
      if (waistVal > 0 && hipVal > 0) {
        ...
        if (isFinite(whrVal)) {
          lines.push(`WHR: ${whrVal.toFixed(2)}`);
        }
      }
    } catch (_) {
...
(function(){
  ...
  ['input','change'].forEach(evt=>{
    document.addEventListener(evt, function(e){
      const t = e.target;
      if(!t || !t.id) return;
      if (['name', 'advName', 'fullName', 'age', 'ageMonths', 'weight', 'height'].includes(t.id)) updateSaveBtnVisibility();
    }, true);
  });
  ...
  window.updatePrevSummaryDiff = function() {
    try {
      ...
      (function(){
        ...
        // Ujednolicenie koloru z oceną wzrostu i masy ciała – BMI nie może wyglądać
        // ani gorzej, ani „zbyt dobrze” w porównaniu z podstawowymi parametrami.
        bmiClass = unifyDiffClass(bmiClass, lastWClass, lastHClass);
        const row = document.createElement('div');
        row.className = 'diff-row';
        const label = document.createElement('div');
        label.className = 'label';
      ...
      (function(){
        ...
        if (coleCurr >= 110) {
          ...
          if (coleDiff > 0) {
            // Dziecko ma niedowagę, ale wskaźnik Cole’a rośnie. Jeżeli jednocześnie
            // wiersze „Wzrost” i „Waga” są ocenione jako prawidłowe (status-ok),
            // nie podbijaj dodatkowo koloru ostrzegawczego – pozostaw turkusowy
            // status-ok dla spójności z oceną podstawowych parametrów.
            if (lastWClass === 'status-ok' && lastHClass === 'status-ok') {
              coleClass = 'status-ok';
            } else {
              coleClass = 'status-improve';
        ...
        coleClass = unifyDiffClass(coleClass, lastWClass, lastHClass);
        const row = document.createElement('div');
        row.className = 'diff-row';
        const label = document.createElement('div');
        label.className = 'label';
