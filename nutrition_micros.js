(function(window, document) {
  ...
  function bindNutritionMicrosProfileListeners() {
    ...
    fieldIds.forEach((id) => {
      ...
      try { node.addEventListener('input', rerender); } catch (_) { /* ignore */ }
      try { node.addEventListener('change', rerender); } catch (_) { /* ignore */ }
    });
  }
  ...
  if (document && typeof document.addEventListener === 'function') {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initMicronormsModule);
    } else {
      initMicronormsModule();
    }
  } else {
    try { initMicronormsModule(); } catch (_) { /* ignore */ }
  }
})(window, document);
