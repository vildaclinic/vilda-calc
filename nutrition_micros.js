(function(window, document) {
  ...
  const MICRONORMS_MODULE_VERSION = '1.2.3';
  ...
  const NUTRITION_MICROS_PROFILE_FIELD_IDS = Object.freeze([
  ...
  function bindNutritionMicrosProfileListeners() {
    NUTRITION_MICROS_PROFILE_FIELD_IDS.forEach((id) => {
      const node = el(id);
      if (!node || node.__nutritionMicrosProfileBound) return;
      node.__nutritionMicrosProfileBound = true;
      const rerender = function(event) {
        try {
          scheduleNutritionMicrosProfileRender(id, event && event.type);
        } catch (_) {
    if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
      globalThis.vildaLogSwallowedCatch('nutrition_micros.js', _, { line: 232, step: '8P-13', helper: 'scheduleNutritionMicrosProfileRender', fieldId: id });
    }
  }
      };
  ...
  const NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP = '8P-14';
  const NUTRITION_MICROS_BOOTSTRAP_RENDER_DELAYS_MS = Object.freeze([0, 60, 180, 500]);
  let nutritionMicrosBootstrapRenderFrameId = null;
  let nutritionMicrosBootstrapRenderPendingSource = null;
  let nutritionMicrosBootstrapRenderPendingDelayMs = null;
  let nutritionMicrosBootstrapRenderPendingTimers = Object.create(null);
  let nutritionMicrosBootstrapRenderScheduledSeriesCount = 0;
  let nutritionMicrosBootstrapRenderScheduledTimerCount = 0;
  let nutritionMicrosBootstrapRenderCoalescedTimerCount = 0;
  let nutritionMicrosBootstrapRenderFrameScheduledCount = 0;
  let nutritionMicrosBootstrapRenderFrameCoalescedCount = 0;
  let nutritionMicrosBootstrapRenderRunCount = 0;
  let nutritionMicrosBootstrapRenderLastResult = null;

  function getNutritionMicrosBootstrapRenderRafScheduler() {
    if (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosBootstrapRenderWithRaf(callback) { return window.requestAnimationFrame(callback); };
    }
    if (typeof requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosBootstrapRenderWithGlobalRaf(callback) { return requestAnimationFrame(callback); };
    }
    return function scheduleNutritionMicrosBootstrapRenderWithTimeout(callback) {
      return setTimeout(function runNutritionMicrosBootstrapRenderFallbackFrame() { callback((typeof Date !== 'undefined' && Date.now) ? Date.now() : 0); }, 0);
    };
  }

  function runNutritionMicrosBootstrapRender(source, delayMs) {
    const renderSource = source ? String(source) : 'bootstrap';
    const renderDelayMs = Number.isFinite(Number(delayMs)) ? Number(delayMs) : null;
    try {
      renderNutritionMicrosCardFromDom();
      nutritionMicrosBootstrapRenderRunCount += 1;
      nutritionMicrosBootstrapRenderLastResult = {
        skipped: false,
        rendered: true,
        scheduled: false,
        source: renderSource,
        delayMs: renderDelayMs,
        step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP
      };
      return nutritionMicrosBootstrapRenderLastResult;
    } catch (_) {
      if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
        globalThis.vildaLogSwallowedCatch('nutrition_micros.js', _, { step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP, source: renderSource, delayMs: renderDelayMs });
      }
      nutritionMicrosBootstrapRenderLastResult = {
        skipped: false,
        rendered: false,
        ...
        delayMs: renderDelayMs,
        step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP,
        error: String(_ && _.message ? _.message : _)
      };
      return nutritionMicrosBootstrapRenderLastResult;
    }
  }

  function scheduleNutritionMicrosBootstrapFrameRender(source, delayMs) {
    const renderSource = source ? String(source) : 'bootstrap';
    const renderDelayMs = Number.isFinite(Number(delayMs)) ? Number(delayMs) : null;
    nutritionMicrosBootstrapRenderPendingSource = renderSource;
    nutritionMicrosBootstrapRenderPendingDelayMs = renderDelayMs;
    if (nutritionMicrosBootstrapRenderFrameId !== null) {
      nutritionMicrosBootstrapRenderFrameCoalescedCount += 1;
      return { skipped: false, rendered: false, scheduled: true, coalesced: true, source: renderSource, delayMs: renderDelayMs, step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP };
    }
    const schedule = getNutritionMicrosBootstrapRenderRafScheduler();
    nutritionMicrosBootstrapRenderFrameScheduledCount += 1;
    nutritionMicrosBootstrapRenderFrameId = schedule(function runCoalescedNutritionMicrosBootstrapRender() {
      const pendingSource = nutritionMicrosBootstrapRenderPendingSource;
      const pendingDelayMs = nutritionMicrosBootstrapRenderPendingDelayMs;
      nutritionMicrosBootstrapRenderFrameId = null;
      nutritionMicrosBootstrapRenderPendingSource = null;
      nutritionMicrosBootstrapRenderPendingDelayMs = null;
      runNutritionMicrosBootstrapRender(pendingSource, pendingDelayMs);
    });
    return { skipped: false, rendered: false, scheduled: true, coalesced: false, source: renderSource, delayMs: renderDelayMs, step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP };
  }

  function scheduleNutritionMicrosBootstrapRender(source) {
    const renderSource = source ? String(source) : 'bootstrap';
    nutritionMicrosBootstrapRenderScheduledSeriesCount += 1;
    NUTRITION_MICROS_BOOTSTRAP_RENDER_DELAYS_MS.forEach((delayMs) => {
      const delayKey = String(delayMs);
      if (Object.prototype.hasOwnProperty.call(nutritionMicrosBootstrapRenderPendingTimers, delayKey)) {
        nutritionMicrosBootstrapRenderCoalescedTimerCount += 1;
        nutritionMicrosBootstrapRenderPendingTimers[delayKey].source = renderSource;
        return;
      }
      nutritionMicrosBootstrapRenderScheduledTimerCount += 1;
      nutritionMicrosBootstrapRenderPendingTimers[delayKey] = { source: renderSource, delayMs };
      setTimeout(function scheduleCoalescedNutritionMicrosBootstrapRenderTimer() {
        const pendingTimer = nutritionMicrosBootstrapRenderPendingTimers[delayKey] || { source: renderSource, delayMs };
        delete nutritionMicrosBootstrapRenderPendingTimers[delayKey];
        scheduleNutritionMicrosBootstrapFrameRender(pendingTimer.source, pendingTimer.delayMs);
      }, delayMs);
    });
    return {
      skipped: false,
      ...
      scheduled: true,
      source: renderSource,
      delaysMs: NUTRITION_MICROS_BOOTSTRAP_RENDER_DELAYS_MS.slice(),
      step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP
    };
  }

  function getNutritionMicrosBootstrapRenderCoalescingSnapshot() {
    return {
      kind: 'vilda-nutrition-micros-bootstrap-render-coalescing-snapshot',
      step: NUTRITION_MICROS_BOOTSTRAP_RENDER_COALESCE_STEP,
      readOnly: true,
      didCallWindowUpdate: false,
      didRenderDom: false,
      scheduler: (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') || typeof requestAnimationFrame === 'function' ? 'requestAnimationFrame' : 'setTimeout',
      delaysMs: NUTRITION_MICROS_BOOTSTRAP_RENDER_DELAYS_MS.slice(),
      pendingTimerDelaysMs: Object.keys(nutritionMicrosBootstrapRenderPendingTimers).map(function(delayKey) { return Number(delayKey); }).filter(function(delayMs) { return Number.isFinite(delayMs); }).sort(function(a, b) { return a - b; }),
      pendingFrame: nutritionMicrosBootstrapRenderFrameId !== null,
      pendingSource: nutritionMicrosBootstrapRenderPendingSource,
      pendingDelayMs: nutritionMicrosBootstrapRenderPendingDelayMs,
      scheduledSeriesCount: nutritionMicrosBootstrapRenderScheduledSeriesCount,
      scheduledTimerCount: nutritionMicrosBootstrapRenderScheduledTimerCount,
      coalescedTimerCount: nutritionMicrosBootstrapRenderCoalescedTimerCount,
      frameScheduledCount: nutritionMicrosBootstrapRenderFrameScheduledCount,
      frameCoalescedCount: nutritionMicrosBootstrapRenderFrameCoalescedCount,
      renderRuns: nutritionMicrosBootstrapRenderRunCount,
      lastResult: nutritionMicrosBootstrapRenderLastResult ? Object.assign({}, nutritionMicrosBootstrapRenderLastResult) : null
    };
  }
  ...
  function bindNutritionMicrosAbbreviationTooltips() {
    ...
    window.addEventListener('scroll', function() { nutritionMicrosHideTooltip(document.activeElement); }, { passive: true, capture: true });
  ...
  const NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP = '8P-9';
  let nutritionMicrosAfterUpdateHookRegistered = false;
  let nutritionMicrosAfterUpdateHookToken = null;
  let nutritionMicrosAfterUpdateFrameId = null;
  let nutritionMicrosAfterUpdatePendingContext = null;
  let nutritionMicrosAfterUpdateScheduledCount = 0;
  let nutritionMicrosAfterUpdateCoalescedCount = 0;
  let nutritionMicrosAfterUpdateRenderRuns = 0;
  let nutritionMicrosAfterUpdateLastResult = null;

  function getNutritionMicrosAfterUpdateRafScheduler() {
    if (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') {
      return function scheduleWithRaf(callback) { return window.requestAnimationFrame(callback); };
    }
    if (typeof requestAnimationFrame === 'function') {
      return function scheduleWithGlobalRaf(callback) { return requestAnimationFrame(callback); };
    }
    return function scheduleWithTimeout(callback) {
      return setTimeout(function runNutritionMicrosAfterUpdateFallbackFrame() { callback((typeof Date !== 'undefined' && Date.now) ? Date.now() : 0); }, 0);
    };
  }

  function runNutritionMicrosAfterUpdateRender(context) {
    try {
      renderNutritionMicrosCardFromDom();
      nutritionMicrosAfterUpdateRenderRuns += 1;
      nutritionMicrosAfterUpdateLastResult = { skipped: false, rendered: true, scheduled: false, step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP };
      return nutritionMicrosAfterUpdateLastResult;
    } catch (_) {
      if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
        globalThis.vildaLogSwallowedCatch('nutrition_micros.js', _, { step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP, hook: NUTRITION_MICROS_AFTER_UPDATE_HOOK_ID });
      }
      nutritionMicrosAfterUpdateLastResult = { skipped: false, rendered: false, scheduled: false, step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP, error: String(_ && _.message ? _.message : _) };
      return nutritionMicrosAfterUpdateLastResult;
    }
  }

  function scheduleNutritionMicrosAfterUpdateRender(context) {
    nutritionMicrosAfterUpdateScheduledCount += 1;
    nutritionMicrosAfterUpdatePendingContext = context || null;
    if (nutritionMicrosAfterUpdateFrameId !== null) {
      nutritionMicrosAfterUpdateCoalescedCount += 1;
      return { skipped: false, rendered: false, scheduled: true, coalesced: true, step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP };
    }
    const schedule = getNutritionMicrosAfterUpdateRafScheduler();
    nutritionMicrosAfterUpdateFrameId = schedule(function runCoalescedNutritionMicrosAfterUpdate() {
      const pendingContext = nutritionMicrosAfterUpdatePendingContext;
      nutritionMicrosAfterUpdateFrameId = null;
      nutritionMicrosAfterUpdatePendingContext = null;
      runNutritionMicrosAfterUpdateRender(pendingContext);
    });
    return { skipped: false, rendered: false, scheduled: true, coalesced: false, step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP };
  }

  function getNutritionMicrosAfterUpdateCoalescingSnapshot() {
    return {
      kind: 'vilda-nutrition-micros-after-update-coalescing-snapshot',
      step: NUTRITION_MICROS_AFTER_UPDATE_COALESCE_STEP,
      readOnly: true,
      didCallWindowUpdate: false,
      didRenderDom: false,
      hookId: NUTRITION_MICROS_AFTER_UPDATE_HOOK_ID,
      scheduler: (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') || typeof requestAnimationFrame === 'function' ? 'requestAnimationFrame' : 'setTimeout',
      pending: nutritionMicrosAfterUpdateFrameId !== null,
      scheduledCount: nutritionMicrosAfterUpdateScheduledCount,
      coalescedCount: nutritionMicrosAfterUpdateCoalescedCount,
      renderRuns: nutritionMicrosAfterUpdateRenderRuns,
      lastResult: nutritionMicrosAfterUpdateLastResult ? Object.assign({}, nutritionMicrosAfterUpdateLastResult) : null
    };
  }
  ...
  function renderNutritionMicrosAfterUpdate(context) {
    ...
    return scheduleNutritionMicrosAfterUpdateRender(context);
  }

  const NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP = '8P-12';
  let nutritionMicrosDirectRenderFrameId = null;
  let nutritionMicrosDirectRenderPendingSource = null;
  let nutritionMicrosDirectRenderScheduledCount = 0;
  let nutritionMicrosDirectRenderCoalescedCount = 0;
  let nutritionMicrosDirectRenderRenderRuns = 0;
  let nutritionMicrosDirectRenderLastResult = null;

  function getNutritionMicrosDirectRenderRafScheduler() {
    if (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosDirectRenderWithRaf(callback) { return window.requestAnimationFrame(callback); };
    }
    if (typeof requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosDirectRenderWithGlobalRaf(callback) { return requestAnimationFrame(callback); };
    }
    return function scheduleNutritionMicrosDirectRenderWithTimeout(callback) {
      return setTimeout(function runNutritionMicrosDirectRenderFallbackFrame() { callback((typeof Date !== 'undefined' && Date.now) ? Date.now() : 0); }, 0);
    };
  }

  function runNutritionMicrosDirectRender(source) {
    const eventSource = source ? String(source) : 'direct-ui-event';
    try {
      renderNutritionMicrosCardFromDom();
      nutritionMicrosDirectRenderRenderRuns += 1;
      nutritionMicrosDirectRenderLastResult = { skipped: false, rendered: true, scheduled: false, source: eventSource, step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP };
      return nutritionMicrosDirectRenderLastResult;
    } catch (_) {
      if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
        globalThis.vildaLogSwallowedCatch('nutrition_micros.js', _, { step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP, source: eventSource });
      }
      nutritionMicrosDirectRenderLastResult = { skipped: false, rendered: false, scheduled: false, source: eventSource, step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP, error: String(_ && _.message ? _.message : _) };
      return nutritionMicrosDirectRenderLastResult;
    }
  }

  function scheduleNutritionMicrosDirectRender(source) {
    const eventSource = source ? String(source) : 'direct-ui-event';
    nutritionMicrosDirectRenderScheduledCount += 1;
    nutritionMicrosDirectRenderPendingSource = eventSource;
    if (nutritionMicrosDirectRenderFrameId !== null) {
      nutritionMicrosDirectRenderCoalescedCount += 1;
      return { skipped: false, rendered: false, scheduled: true, coalesced: true, source: eventSource, step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP };
    }
    const schedule = getNutritionMicrosDirectRenderRafScheduler();
    nutritionMicrosDirectRenderFrameId = schedule(function runCoalescedNutritionMicrosDirectRender() {
      const pendingSource = nutritionMicrosDirectRenderPendingSource;
      nutritionMicrosDirectRenderFrameId = null;
      nutritionMicrosDirectRenderPendingSource = null;
      runNutritionMicrosDirectRender(pendingSource);
    });
    return { skipped: false, rendered: false, scheduled: true, coalesced: false, source: eventSource, step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP };
  }

  function getNutritionMicrosDirectRenderCoalescingSnapshot() {
    return {
      kind: 'vilda-nutrition-micros-direct-render-coalescing-snapshot',
      step: NUTRITION_MICROS_DIRECT_RENDER_COALESCE_STEP,
      readOnly: true,
      didCallWindowUpdate: false,
      didRenderDom: false,
      scheduler: (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') || typeof requestAnimationFrame === 'function' ? 'requestAnimationFrame' : 'setTimeout',
      coalescedSources: ['nutritionMicrosDataReady', 'vildaResultsModeChanged'],
      pending: nutritionMicrosDirectRenderFrameId !== null,
      pendingSource: nutritionMicrosDirectRenderPendingSource,
      scheduledCount: nutritionMicrosDirectRenderScheduledCount,
      coalescedCount: nutritionMicrosDirectRenderCoalescedCount,
      renderRuns: nutritionMicrosDirectRenderRenderRuns,
      lastResult: nutritionMicrosDirectRenderLastResult ? Object.assign({}, nutritionMicrosDirectRenderLastResult) : null
    };
  }


  const NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP = '8P-13';
  let nutritionMicrosProfileRenderFrameId = null;
  let nutritionMicrosProfileRenderPendingFieldId = null;
  let nutritionMicrosProfileRenderPendingEventType = null;
  let nutritionMicrosProfileRenderScheduledCount = 0;
  let nutritionMicrosProfileRenderCoalescedCount = 0;
  let nutritionMicrosProfileRenderRenderRuns = 0;
  let nutritionMicrosProfileRenderLastResult = null;

  function getNutritionMicrosProfileRenderRafScheduler() {
    if (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosProfileRenderWithRaf(callback) { return window.requestAnimationFrame(callback); };
    }
    if (typeof requestAnimationFrame === 'function') {
      return function scheduleNutritionMicrosProfileRenderWithGlobalRaf(callback) { return requestAnimationFrame(callback); };
    }
    return function scheduleNutritionMicrosProfileRenderWithTimeout(callback) {
      return setTimeout(function runNutritionMicrosProfileRenderFallbackFrame() { callback((typeof Date !== 'undefined' && Date.now) ? Date.now() : 0); }, 0);
    };
  }

  function normalizeNutritionMicrosProfileEventType(eventType) {
    const type = eventType ? String(eventType) : 'profile-input';
    return type === 'change' ? 'change' : 'input';
  }

  function runNutritionMicrosProfileRender(fieldId, eventType) {
    const pendingFieldId = fieldId ? String(fieldId) : null;
    const pendingEventType = normalizeNutritionMicrosProfileEventType(eventType);
    try {
      renderNutritionMicrosCardFromDom();
      nutritionMicrosProfileRenderRenderRuns += 1;
      nutritionMicrosProfileRenderLastResult = {
        skipped: false,
        rendered: true,
        scheduled: false,
        fieldId: pendingFieldId,
        eventType: pendingEventType,
        step: NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP
      };
      return nutritionMicrosProfileRenderLastResult;
    } catch (_) {
      if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
        globalThis.vildaLogSwallowedCatch('nutrition_micros.js', _, { step: NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP, fieldId: pendingFieldId, eventType: pendingEventType });
      }
      nutritionMicrosProfileRenderLastResult = {
        skipped: false,
        rendered: false,
  ...
  function scheduleNutritionMicrosProfileRender(fieldId, eventType) {
    const pendingFieldId = fieldId ? String(fieldId) : null;
    const pendingEventType = normalizeNutritionMicrosProfileEventType(eventType);
    nutritionMicrosProfileRenderScheduledCount += 1;
    nutritionMicrosProfileRenderPendingFieldId = pendingFieldId;
    nutritionMicrosProfileRenderPendingEventType = pendingEventType;
    if (nutritionMicrosProfileRenderFrameId !== null) {
      nutritionMicrosProfileRenderCoalescedCount += 1;
      return { skipped: false, rendered: false, scheduled: true, coalesced: true, fieldId: pendingFieldId, eventType: pendingEventType, step: NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP };
    }
    const schedule = getNutritionMicrosProfileRenderRafScheduler();
    nutritionMicrosProfileRenderFrameId = schedule(function runCoalescedNutritionMicrosProfileRender() {
      const pendingFieldIdForFrame = nutritionMicrosProfileRenderPendingFieldId;
      const pendingEventTypeForFrame = nutritionMicrosProfileRenderPendingEventType;
      nutritionMicrosProfileRenderFrameId = null;
      nutritionMicrosProfileRenderPendingFieldId = null;
      nutritionMicrosProfileRenderPendingEventType = null;
      runNutritionMicrosProfileRender(pendingFieldIdForFrame, pendingEventTypeForFrame);
    });
    return { skipped: false, rendered: false, scheduled: true, coalesced: false, fieldId: pendingFieldId, eventType: pendingEventType, step: NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP };
  }

  function getNutritionMicrosProfileRenderCoalescingSnapshot() {
    return {
      kind: 'vilda-nutrition-micros-profile-render-coalescing-snapshot',
      step: NUTRITION_MICROS_PROFILE_RENDER_COALESCE_STEP,
      readOnly: true,
      didCallWindowUpdate: false,
      didRenderDom: false,
      scheduler: (typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function') || typeof requestAnimationFrame === 'function' ? 'requestAnimationFrame' : 'setTimeout',
      coalescedEventTypes: ['input', 'change'],
      fieldIds: NUTRITION_MICROS_PROFILE_FIELD_IDS.slice(),
      pending: nutritionMicrosProfileRenderFrameId !== null,
      pendingFieldId: nutritionMicrosProfileRenderPendingFieldId,
      pendingEventType: nutritionMicrosProfileRenderPendingEventType,
      scheduledCount: nutritionMicrosProfileRenderScheduledCount,
      coalescedCount: nutritionMicrosProfileRenderCoalescedCount,
      renderRuns: nutritionMicrosProfileRenderRenderRuns,
      lastResult: nutritionMicrosProfileRenderLastResult ? Object.assign({}, nutritionMicrosProfileRenderLastResult) : null
    };
  }
  ...
  function getNutritionMicrosUpdateHookSnapshot(options) {
    ...
    return {
      ...
      afterUpdateCoalescing: getNutritionMicrosAfterUpdateCoalescingSnapshot(),
      directRenderCoalescing: getNutritionMicrosDirectRenderCoalescingSnapshot(),
      profileRenderCoalescing: getNutritionMicrosProfileRenderCoalescingSnapshot(),
      bootstrapRenderCoalescing: getNutritionMicrosBootstrapRenderCoalescingSnapshot(),
      nextStep: '8P-15 — modularizacja app.js bez zmian klinicznych'
    };
  }
  ...
  function initMicronormsModule() {
    ...
    if (MICRONORMS_INIT_DONE) {
      scheduleNutritionMicrosBootstrapRender('init-repeat');
      return;
    }
    ...
    try {
      window.addEventListener('nutritionMicrosDataReady', function() {
        scheduleNutritionMicrosDirectRender('nutritionMicrosDataReady');
      });
    } catch (_) {
    if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
    ...
    try {
      window.addEventListener('vildaResultsModeChanged', function() {
        scheduleNutritionMicrosDirectRender('vildaResultsModeChanged');
      });
    } catch (_) {
    if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
    ...
    try {
      window.addEventListener('pageshow', function() {
        bindNutritionMicrosProfileListeners();
        scheduleNutritionMicrosBootstrapRender('pageshow');
      });
    } catch (_) {
    if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
    ...
    try {
      const bootOnWindowLoad = function() {
        bindNutritionMicrosProfileListeners();
        scheduleNutritionMicrosBootstrapRender('window-load');
      };
    ...
    scheduleNutritionMicrosBootstrapRender('init');
  }
  ...
  window.vildaGetNutritionMicrosAfterUpdateCoalescingSnapshot = getNutritionMicrosAfterUpdateCoalescingSnapshot;
  window.vildaGetNutritionMicrosDirectRenderCoalescingSnapshot = getNutritionMicrosDirectRenderCoalescingSnapshot;
  window.vildaGetNutritionMicrosProfileRenderCoalescingSnapshot = getNutritionMicrosProfileRenderCoalescingSnapshot;
  window.vildaGetNutritionMicrosBootstrapRenderCoalescingSnapshot = getNutritionMicrosBootstrapRenderCoalescingSnapshot;
  window.VildaNutritionMicros = Object.freeze({
    ...
    getAfterUpdateCoalescingSnapshot: getNutritionMicrosAfterUpdateCoalescingSnapshot,
    getDirectRenderCoalescingSnapshot: getNutritionMicrosDirectRenderCoalescingSnapshot,
    getProfileRenderCoalescingSnapshot: getNutritionMicrosProfileRenderCoalescingSnapshot,
    getBootstrapRenderCoalescingSnapshot: getNutritionMicrosBootstrapRenderCoalescingSnapshot,
