...
Stary kod interpolacji liniowej i wygładzania pozostaje w plikach jako fallback wykonywany tylko wtedy, gdy `vilda_centile_interpolation.js` nie jest załadowany. Preferencja `PAL_SMOOTH_PASSES` działa wyłącznie w ścieżce fallbacku. Od kroku 2 konsolidacji (2026-08-11) moduł jest ładowany także w `docpro.html`, a kopie generatorów siatek `inline_docpro_01/02/05.js` delegują interpolację Palczewskiej i pomijają wygładzanie uśrednianiem identycznie jak ich odpowiedniki na `index.html` (`inline_index_03/04/07.js`).
