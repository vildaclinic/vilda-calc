...
(function (global) {
  ...
  function syncPersistLockFlags(root) {
    ...
    try {
      // Bug 1 fix vol2 (2026-05): sexLocked jest CZYSTYM mirror'em
      // `sex.disabled` w DOM. Wcześniej guard z sexValue nie działał, bo
      // `<select id="sex">` ma default `<option value="M">` więc value zawsze
      // jest 'M' (truthy) — stale `sexLocked: true` ze storage zapisywało się
      // przy KAŻDYM save, bez względu na to czy pacjent był faktycznie
      // załadowany. Teraz: jeśli element NIE jest disabled w DOM, kasujemy
      // stale flag z root.
      const sexEl = document.getElementById('sex');
      const sexLocked = !!(sexEl && sexEl.disabled);
      if (sexLocked) {
        root.sexLocked = true;
      } else if (root && root.sexLocked) {
  ...
  function applyPersistLockFlags(root) {
    ...
    try {
      if (root && root.sexLocked) {
        const sexEl = document.getElementById('sex');
        // Bug 1 fix vol2 (2026-05): blokujemy sex TYLKO gdy mamy faktycznie
        // załadowanego pacjenta (root.name lub root.nameLocked). Domyślny
        // `value="M"` selectu sex sprawiał że poprzedni guard "tylko gdy
        // ma wartość" zawsze przepuszczał. Teraz jasny sygnał: bez nazwy
        // pacjenta = brak load'u = nie ma czego blokować. Stale flag
        // czyścimy z storage, żeby kolejne nawigacje nie próbowały blokować.
        const hasLoadedPatient = !!(root.name || root.nameLocked);
        if (sexEl && hasLoadedPatient) {
          sexEl.disabled = true;
        } else {
          delete root.sexLocked;
  ...
  try {
    ...
    if (typeof globalThis !== 'undefined' && typeof globalThis.vildaLogSwallowedCatch === 'function') {
      ...
      if (sexEl) {
        ...
        try {
          // Bug 1 fix vol2 (2026-05): blokujemy sex tylko jeśli mamy faktycznie
          // załadowanego pacjenta (root.name lub root.nameLocked). Sex sam
          // ma default "M" więc ocenianie po sex.value zawsze przepuszczało.
          const hasLoadedPatient = !!(root && (root.name || root.nameLocked));
          if (root && root.sexLocked && hasLoadedPatient) {
            sexEl.disabled = true;
          } else if (root && root.sexLocked) {
            // Stale flag — skasuj go i odblokuj pole.
            delete root.sexLocked;
