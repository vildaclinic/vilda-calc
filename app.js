...
planResults.innerHTML  = '';
  ...
  if(bmiReady){
    ...
    /* --- BSA Haycock tylko gdy wiek < 18 -------------------------------- */
    let bsa = null;
    if(age < 18){
    bsa = 0.024265 * Math.pow(weight,0.5378) * Math.pow(height,0.3964);
    bsa = Math.round(bsa*100)/100;   // 2 miejsca po przecinku
    }
...
/* ===== KONIEC KARTY CENTYLI ===== */
    html += `<div class="result-box">
      <strong>BMI: <span class="result-val">${bmiText}</span>${percText} (${bmiCat})</strong>${anorexiaNote}
      ${bsa!==null ? `<br><strong class="result-number--small">Pow. ciała (BSA): <span class="result-number">${bsa.toFixed(2)} m²</span></strong>` : ''}
    </div>`;
    if(bmrReady){
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',transformPlan);
    /* === KEEP HEIGHT INPUT VISIBLE ON MOBILE ================= */
    ...
    if (heightInput){
      ...
      heightInput.addEventListener('focus', ()=>{
        ...
        document.body.classList.add('keyboard-space'); // padding-bottom 60vh
        setTimeout(()=>{                               // po wyskoczeniu klawiatury
          heightInput.scrollIntoView({block:'center', behavior:'smooth'});
        }, 300);
      });
  
      heightInput.addEventListener('blur', ()=>{
        document.body.classList.remove('keyboard-space');
      });
    }  
})();
