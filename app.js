...
/* === GLOBAL SETTINGS === */
const settings = { bmiSource: 'OLAF' };   // domyślnie OLAF
...
// --- obsługa suwaka źródła BMI ---
const toggle = document.getElementById('bmiToggle');
const toggleWrap = document.getElementById('bmiSourceToggle');

if (age >= 18){             // dorośli – ukryj całkiem
  toggleWrap.style.display = 'none';
  settings.bmiSource = 'WHO';   // nie ma znaczenia, ale jawnie
} else if (age < 7){        // <7 l. – OLAF niedostępny
  toggleWrap.style.display = '';
  toggle.disabled = true;
  toggle.checked = true;    // prawa pozycja = WHO
  settings.bmiSource = 'WHO';
} else {                    // 7–17 l. – suwak aktywny
  toggleWrap.style.display = '';
  toggle.disabled = false;
  settings.bmiSource = toggle.checked ? 'WHO' : 'OLAF';
}
...
function getLMS(sex, months){
  ...
  // 3a.  Zakres OLAF 7–18 l. (84‑216 mies.)
  if (settings.bmiSource === 'OLAF' && m >= 84 && m <= 216){
    const src = sex === 'M' ? LMS_BMI_OLAF_BOYS : LMS_BMI_OLAF_GIRLS;
    if (src[m]) return [src[m].L, src[m].M, src[m].S];
  }
  // 3b.  WHO infant (0‑60 m) + WHO 5‑19 l.
  if (m <= 60){
...
/* ==== BMI – OLAF 2010 (7–18 lat) – klucz = wiek (mies.) ==== */
const LMS_BMI_OLAF_BOYS = {
  84:{L:-2.22,M:15.8,S:0.119},96:{L:-2.16,M:16.2,S:0.129},
  108:{L:-2.09,M:16.7,S:0.140},120:{L:-2.03,M:17.1,S:0.148},
  132:{L:-1.96,M:17.6,S:0.154},144:{L:-1.88,M:18.1,S:0.155},
  156:{L:-1.80,M:18.6,S:0.152},168:{L:-1.72,M:19.2,S:0.146},
  180:{L:-1.63,M:19.8,S:0.140},192:{L:-1.53,M:20.4,S:0.136},
  204:{L:-1.44,M:21.1,S:0.134},216:{L:-1.34,M:21.8,S:0.133}
};
const LMS_BMI_OLAF_GIRLS = {
  84:{L:-2.06,M:15.5,S:0.119},96:{L:-1.93,M:16.0,S:0.129},
  108:{L:-1.79,M:16.4,S:0.139},120:{L:-1.66,M:16.9,S:0.147},
  132:{L:-1.53,M:17.5,S:0.153},144:{L:-1.44,M:18.1,S:0.154},
  156:{L:-1.42,M:18.8,S:0.150},168:{L:-1.46,M:19.4,S:0.143},
  180:{L:-1.54,M:19.9,S:0.136},192:{L:-1.62,M:20.3,S:0.130},
  204:{L:-1.69,M:20.5,S:0.126},216:{L:-1.74,M:20.7,S:0.124}
};
...
(function(){
  ...
  obs.observe(document.body,{childList:true,subtree:true});
  document.getElementById('bmiToggle').addEventListener('change', () => update());
