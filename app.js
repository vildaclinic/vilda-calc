...
if (typeof window !== 'undefined' && typeof window.vildaAppOnReady === 'function') {
  window.vildaAppOnReady('app:auto-scroll-focus-exclusions', function initAutoScrollFocusExclusions() {
    if (isVildaAutoScrollPermanentlyDisabled()) return;
    // Listener instalujemy RAZ (Etap 4.2). Bez tego każda Turbo-nawigacja
    // dodawała kolejnego document.focusin handlera — przy 5 nawigacjach
    // każdy focus w pole wagaiwzrost'a wykonywał 5 kopii logiki.
    if (typeof window.vildaInstallGlobalListener !== 'function') return;
    window.vildaInstallGlobalListener(
      'app:auto-scroll-focus-exclusions:focusin',
      document,
      'focusin',
      function (e) {
        try {
...
// Listener instalowany RAZ — vildaInstallGlobalListener (Etap 4.2). Bez tego
// każda re-evaluacja tego skryptu (Turbo NIE re-evaluuje przy data-turbo-track="reload"
// jeśli URL skryptu jest taki sam, ale w niektórych edge-case'ach jak SW update
// może dojść do dwukrotnej rejestracji) dodawała kolejny resize handler.
if (typeof window.vildaInstallGlobalListener === 'function') {
  window.vildaInstallGlobalListener('app:main-layout-resize', window, 'resize', function () {
...
// Anti-leak: instalujemy raz (Etap 4.2)
if (typeof window.vildaInstallGlobalListener === 'function') {
  window.vildaInstallGlobalListener('app:test-button-width-resize', window, 'resize', function () {
...
function openIntakeCard(options){
  ...
  // Anti-leak (Etap 4.2): instalujemy raz, niezależnie od ile razy
  // setupEstimatedIntake jest re-fired przez vildaAppOnReady przy turbo:load.
  if (typeof window.vildaInstallGlobalListener === 'function') {
    window.vildaInstallGlobalListener('app:estimated-intake-resize', window, 'resize', function () {
      var c = document.getElementById('intakeCard');
      var visible = c && c.style.display !== 'none';
...
(function(){
  ...
  // Anti-leak (Etap 4.2): instalujemy raz, niezależnie od tego ile razy IIFE
  // jest re-evaluowany. Capture-phase listenery na document mnożyły się przy
  // każdej Turbo-nawigacji.
  function handleAppGlobalInputChange(e) {
    const t = e.target;
    if(!t) return;
    try {
  ...
  if (typeof window !== 'undefined' && typeof window.vildaInstallGlobalListener === 'function') {
    window.vildaInstallGlobalListener('app:save-btn-visibility:input', document, 'input', handleAppGlobalInputChange, true);
    window.vildaInstallGlobalListener('app:save-btn-visibility:change', document, 'change', handleAppGlobalInputChange, true);
  } else {
    ['input','change'].forEach(evt=>{
      document.addEventListener(evt, handleAppGlobalInputChange, true);
    });
  }
  ...
  window.vildaAppOnReady('app:mobile-menu-autocollapse', function initMobileMenuAutocollapse() {
    // Anti-leak (Etap 4.2): zarówno navToggle change, jak i document click są
    // instalowane RAZ. navToggle siedzi w `data-turbo-permanent` headerze,
    // więc element przeżywa nawigację — kolejne re-fire'y dodawały kolejne
    // listenery na ten sam permanentny element. Po N nawigacjach każde
    // tappnięcie hamburgera triggerowało N kopii handlera.
    if (typeof window.vildaInstallGlobalListener !== 'function') return;

    window.vildaInstallGlobalListener(
      'app:mobile-menu-autocollapse:nav-toggle-change',
      document,
      'change',
      function (event) {
        var navToggle = document.getElementById('navToggle');
        if (!navToggle || event.target !== navToggle) return;
    ...
    window.vildaInstallGlobalListener(
      'app:mobile-menu-autocollapse:doc-click',
      document,
      'click',
      function (event) {
        var navToggle = document.getElementById('navToggle');
        var verticalMenu = document.getElementById('verticalMenu');
        var toggleLabel = document.querySelector('label[for="navToggle"]');
        if (!navToggle || !verticalMenu) return;
