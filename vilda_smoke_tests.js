/*
 * Vilda Smoke Regression Suite v2.37.0
 *
...
(function (global) {
  ...
  const DEFAULT_GROWTH_REFERENCE_DATA_CONTRACTS = Object.freeze([
    'growth-reference-data'
  ]);
  const DEFAULT_PROFESSIONAL_MODULE_CONTRACTS = Object.freeze([
    'professional-module'
  ]);
  const DEFAULT_PERSIST_RUNTIME_CONTRACTS = Object.freeze([
    'persist-runtime'
  ]);
  const DEFAULT_SUMMARY_CARDS_CONTRACTS = Object.freeze([
    'summary-cards'
  ]);
  const DEFAULT_DIET_PLAN_UI_CONTRACTS = Object.freeze([
    'diet-plan-ui'
  ]);
  const DEFAULT_ESTIMATED_INTAKE_UI_CONTRACTS = Object.freeze([
    'estimated-intake-ui'
  ]);
  const DEFAULT_ESTIMATED_INTAKE_RUNTIME_CONTRACTS = Object.freeze([
    'estimated-intake-runtime'
  ]);
  const DEFAULT_ESTIMATED_INTAKE_INPUT_MODEL_CONTRACTS = Object.freeze([
    'estimated-intake-input-model'
  ]);
  const DEFAULT_ESTIMATED_INTAKE_DOM_MOUNT_CONTRACTS = Object.freeze([
    'estimated-intake-dom-mount'
  ]);
  ...
  const EXPECTED_BROWSER_SCRIPTS = Object.freeze([
    ...
    'vilda_data_import_export.js?v=66',
    ...
    'vilda_diet_recommendations.js?v=22',
  ...
  const MANIFEST = Object.freeze([
    ...
    Object.freeze({ id: 'estimated-intake-ui', group: 'estimated-intake-ui', required: true, description: 'Renderer wyników estimated intake jest wydzielony z app.js i buduje HTML bez DOM oraz bez zapisu window.*.' }),
    Object.freeze({ id: 'estimated-intake-runtime', group: 'estimated-intake-runtime', required: true, description: 'Runtime efektów ubocznych estimated intake jest wydzielony z app.js i deleguje commit window.* oraz post-render risk checks.' }),
    Object.freeze({ id: 'estimated-intake-input-model', group: 'estimated-intake-input-model', required: true, description: 'Model wejściowy i observed estimated intake są wydzielone z app.js do adaptera DI bez renderowania DOM i bez zapisu window.*.' }),
    ...
    Object.freeze({ id: 'growth-reference-data-module', group: 'growth-reference-data', required: true, description: 'Statyczne dane BMI/LMS/WHO-WFL są wydzielone do modułu data-only i udostępniają read-only snapshot.' }),
    Object.freeze({ id: 'professional-module', group: 'professional-module', required: true, description: 'Moduł profesjonalny jest wydzielony z app.js i udostępnia API init oraz read-only snapshot.' }),
    Object.freeze({ id: 'persist-runtime', group: 'persist-runtime', required: true, description: 'Runtime persistence/autosave/restore jest wydzielony z app.js i udostępnia API init oraz read-only snapshot.' }),
    Object.freeze({ id: 'summary-cards', group: 'summary-cards', required: true, description: 'Karty podsumowań/metabolic summary UI są wydzielone z app.js i udostępniają API init oraz read-only snapshot.' }),
    Object.freeze({ id: 'diet-plan-ui', group: 'diet-plan-ui', required: true, description: 'Diet plan / energy plan renderer jest wydzielony z app.js i udostępnia API init oraz read-only snapshot.' }),
    ...
    Object.freeze({ id: 'update-hooks-nutrition-micros-wrapper', group: 'update-hooks', required: true, description: 'Wrapper window.update z nutrition_micros.js jest przepięty na VildaUpdateHooks, a rendery UI/bootstrap mikroelementów mają audyt rAF-koalescencji.' }),
    ...
    Object.freeze({ id: 'pwa-manifest-icon-cache-audit', group: 'pwa-cache', required: true, description: 'Manifest PWA ma start_url/scope w root, shortcuty w scope, materializowane ikony oraz pokrycie cache Service Workera.' }),
    Object.freeze({ id: 'script-cache-versions', group: 'cache-and-load-order', required: true, description: 'HTML/SW ładują oczekiwane wersje skryptów po 8Q-6.' }),
  ...
  function checkGrowthReferenceDataModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaGrowthReferenceData');
    const snapshotFn = resolvePath('vildaGetGrowthReferenceDataSnapshot');
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    const contractDetail = checkNamedContracts(DEFAULT_GROWTH_REFERENCE_DATA_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });
    const datasets = snapshot && snapshot.datasets ? snapshot.datasets : {};
    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-1' &&
        ...
        !!(snapshot && snapshot.readOnly === true && snapshot.dataOnly === true && snapshot.didRenderDom === false && snapshot.didCallWindowUpdate === false) &&
        !!(datasets.WFL_DATA_GIRLS && datasets.WFL_DATA_GIRLS.count === 131) &&
        !!(datasets.WFL_DATA_BOYS && datasets.WFL_DATA_BOYS.count === 131) &&
        !!(datasets.bmiPercentiles && datasets.bmiPercentiles.boysCount === 205 && datasets.bmiPercentiles.girlsCount === 205) &&
        !!(datasets.LMS_BOYS && datasets.LMS_BOYS.count === 205) &&
        !!(datasets.LMS_GIRLS && datasets.LMS_GIRLS.count === 205) &&
        !!(datasets.LMS_INFANT_BOYS && datasets.LMS_INFANT_BOYS.count === 61) &&
        !!(datasets.LMS_INFANT_GIRLS && datasets.LMS_INFANT_GIRLS.count === 61) &&
        !!(datasets.bmiInfantBoys && datasets.bmiInfantBoys.count === 61) &&
        !!(datasets.bmiInfantGirls && datasets.bmiInfantGirls.count === 61) &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
      snapshot: snapshot ? clonePlain(snapshot, 5) : null,
      contractDetail
    };
  }

  function checkProfessionalModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaProfessionalModule');
    const snapshotFn = resolvePath('vildaGetProfessionalModuleSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    const contractDetail = checkNamedContracts(DEFAULT_PROFESSIONAL_MODULE_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });
    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-2' &&
        typeof api.init === 'function' &&
        typeof api.initProfessionalModule === 'function' &&
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didCallWindowUpdate === false) &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
  ...
  function checkPersistRuntime(options) {
    const opts = options || {};
    const api = resolvePath('VildaPersistRuntime');
    const snapshotFn = resolvePath('vildaGetPersistRuntimeSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    ...
    return {
      ok: !!api &&
        ...
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        typeof resolvePath('vildaPersistScheduleSave') === 'function' &&
        typeof resolvePath('vildaPersistFlushNow') === 'function' &&
        typeof resolvePath('vildaGetPersistAutosaveCoalescingSnapshot') === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didCallWindowUpdate === false) &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
  ...
  function checkSummaryCardsModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaSummaryCards');
    const snapshotFn = resolvePath('vildaGetSummaryCardsSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    const contractDetail = checkNamedContracts(DEFAULT_SUMMARY_CARDS_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });
    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-4' &&
        typeof api.init === 'function' &&
        typeof api.initSummaryCards === 'function' &&
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        typeof resolvePath('generateMetabolicSummary') === 'function' &&
        typeof resolvePath('handleMetabolicSummaryClick') === 'function' &&
        typeof resolvePath('__renderPrevSummary') === 'function' &&
        typeof resolvePath('__renderPrevClcrSummary') === 'function' &&
        typeof resolvePath('repositionDoctor') === 'function' &&
        typeof resolvePath('repositionMetabolicSummary') === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didWriteStorage === false) &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
      snapshot: snapshot ? clonePlain(snapshot, 4) : null,
  ...
  function checkDietPlanUiModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaDietPlanUI');
    const snapshotFn = resolvePath('vildaGetDietPlanUiSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    ...
    return {
      ok: !!api &&
        ...
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        typeof resolvePath('updatePlanFromDiet') === 'function' &&
        typeof resolvePath('fillDietSelect') === 'function' &&
        typeof resolvePath('updateDietDescription') === 'function' &&
        typeof resolvePath('updatePalDescription') === 'function' &&
        typeof resolvePath('BMR') === 'function' &&
        typeof resolvePath('energyBuildContext') === 'function' &&
        typeof resolvePath('energyBuildPlanReductionState') === 'function' &&
        typeof resolvePath('energyBuildIntakeObservedState') === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didWriteStorage === false && snapshot.didCallWindowUpdate === false) &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
      snapshot: snapshot ? clonePlain(snapshot, 4) : null,
  ...
  function checkEstimatedIntakeUiModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaEstimatedIntakeUI');
    const snapshotFn = resolvePath('vildaGetEstimatedIntakeUiSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    const deps = { dependencies: { energyRenderModeBadgeHtml: function () { return '<span data-smoke-mode-badge="1">badge</span>'; } } };
    const before = captureWindowState();
    const empty = api && typeof api.buildResultsHtml === 'function' ? api.buildResultsHtml({ rows: [], rowCount: 0, branch: 'empty-rows-message' }, deps) : null;
    const maintenance = api && typeof api.buildResultsHtml === 'function' ? api.buildResultsHtml({
      rows: [{ ageYears: 5, ageMonths: 60, months: 60, weight: 20, height: 110 }],
      rowCount: 1,
      branch: 'single-row-maintenance',
      single: { kind: 'maintenance', maintenanceKcal: 1320, palUsed: 1.4, energy: { modeBadge: { mode: 'actual' } } }
    }, deps) : null;
    const infantUnder6 = api && typeof api.buildResultsHtml === 'function' ? api.buildResultsHtml({
      rows: [{ ageYears: 0.3, ageMonths: 4, months: 4, weight: 6, height: 62 }],
      rowCount: 1,
      branch: 'single-row-maintenance',
      single: { kind: 'infant-under-6', energy: { modeBadge: { mode: 'infant' } } }
    }, deps) : null;
    const infantButte = api && typeof api.buildResultsHtml === 'function' ? api.buildResultsHtml({
      rows: [{ ageYears: 0.7, ageMonths: 8, months: 8, weight: 8, height: 70 }],
      rowCount: 1,
      branch: 'single-row-maintenance',
      single: { kind: 'infant-butte', teeRawKcal: 720, energy: { modeBadge: { mode: 'butte' } } }
    }, deps) : null;
    const multi = api && typeof api.buildResultsHtml === 'function' ? api.buildResultsHtml({
      rows: [
        { ageYears: 5, ageMonths: 60, months: 60, weight: 20, height: 110 },
        { ageYears: 5.5, ageMonths: 66, months: 66, weight: 22, height: 113 }
      ],
      ...
      modeBadge: { mode: 'actual' },
      intervals: [{ from: 5, to: 5.5, days: 183, dW: 2, expectedGain: 1.2, deltaVsNorm: 0.8, energyDeltaPerDay: 34, intakePerDay: 1480, isChild: true }],
      hasChildIntervals: true
    }, deps) : null;
    const after = captureWindowState();
    const contractDetail = checkNamedContracts(DEFAULT_ESTIMATED_INTAKE_UI_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });
    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-6' &&
        typeof api.buildResultsHtml === 'function' &&
        typeof api.buildEmptyResultsHtml === 'function' &&
        typeof api.buildSingleRowResultsHtml === 'function' &&
        typeof api.buildIntervalResultsHtml === 'function' &&
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
      ...
      snapshot: snapshot ? clonePlain(snapshot, 4) : null,
      samples: {
        empty: empty ? { branch: empty.branch, legendVisible: empty.legendVisible, htmlLength: empty.html.length } : null,
        maintenance: maintenance ? { branch: maintenance.branch, hasModeBadge: maintenance.hasModeBadge, htmlLength: maintenance.html.length } : null,
        infantUnder6: infantUnder6 ? { branch: infantUnder6.branch, htmlLength: infantUnder6.html.length } : null,
        infantButte: infantButte ? { branch: infantButte.branch, htmlLength: infantButte.html.length } : null,
        multi: multi ? { branch: multi.branch, legendVisible: multi.legendVisible, intervalCount: multi.intervalCount, htmlLength: multi.html.length } : null
      },
      sideEffectsChanged: stateChanged(before, after),
      contractDetail
    };
  }

  function checkEstimatedIntakeRuntimeModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaEstimatedIntakeRuntime');
    const snapshotFn = resolvePath('vildaGetEstimatedIntakeRuntimeSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    ...
    const history = api && typeof api.buildEstimatedIntakeWindowHistory === 'function' ? api.buildEstimatedIntakeWindowHistory(sampleRows) : null;
    const root = { intakeHistory: 'stale', intakeEstimatedKcalPerDay: 999 };
    const clear = api && typeof api.clearEstimatedIntakeWindowState === 'function' ? api.clearEstimatedIntakeWindowState({ root }) : null;
    const commit = api && typeof api.commitEstimatedIntakeWindowState === 'function' ? api.commitEstimatedIntakeWindowState(sampleRows, 1480, { root }) : null;
    const calcCommit = api && typeof api.commitEstimatedIntakeCalcModelWindowState === 'function' ? api.commitEstimatedIntakeCalcModelWindowState({
      rows: sampleRows,
      commitPlan: { action: 'set', rows: sampleRows, intakeKcalPerDay: 1480 }
    }, { root }) : null;

    const calls = [];
    const riskRoot = { intakeHistory: history || [], intakeEstimatedKcalPerDay: 1480 };
    const riskResult = api && typeof api.runEstimatedIntakePostRenderRisk === 'function' ? api.runEstimatedIntakePostRenderRisk({
      rows: sampleRows,
      postRenderRiskPlan: {
        shouldRun: true,
        row: sampleRows[1],
        ...
        sex: 'M',
        pal: 1.4,
        intakeKcalPerDay: 1480
      }
    }, {
      root: riskRoot,
      mountId: 'intakeResults',
      dependencies: {
        energyBuildIntakeObservedState: function (payload) { calls.push({ type: 'energy', payload }); return { ok: true }; },
        check12mLossOrange: function (hist, mountId) { calls.push({ type: 'check12m', hist, mountId }); },
        logSwallowedCatch: function () {}
      }
    }) : null;
    const skippedRisk = api && typeof api.runEstimatedIntakePostRenderRisk === 'function' ? api.runEstimatedIntakePostRenderRisk({}, { root: riskRoot, dependencies: { energyBuildIntakeObservedState: function () {} } }) : null;
    const after = captureWindowState();
    const contractDetail = checkNamedContracts(DEFAULT_ESTIMATED_INTAKE_RUNTIME_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });

    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-7' &&
        typeof api.buildEstimatedIntakeWindowHistory === 'function' &&
        typeof api.clearEstimatedIntakeWindowState === 'function' &&
        typeof api.commitEstimatedIntakeWindowState === 'function' &&
        typeof api.commitEstimatedIntakeCalcModelWindowState === 'function' &&
        typeof api.runEstimatedIntakePostRenderRisk === 'function' &&
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didWriteStorage === false && snapshot.canWriteWindowState === true) &&
        !!(history && history.length === 2 && history[0].ageMonths === 60 && history[0].height === 110) &&
        !!(clear && root.intakeHistory !== 'stale' && root.intakeEstimatedKcalPerDay !== 999) &&
        !!(commit && Array.isArray(root.intakeHistory) && root.intakeHistory.length === 2 && root.intakeEstimatedKcalPerDay === 1480) &&
        !!(calcCommit && Array.isArray(root.intakeHistory) && root.intakeEstimatedKcalPerDay === 1480) &&
        !!(riskResult && riskResult.didRun === true && riskResult.didRunEnergyObservedState === true && riskResult.didRunCheck12mLossOrange === true) &&
        calls.some(function (call) { return call.type === 'energy' && call.payload && call.payload.mountId === 'intakeResults' && call.payload.applyRiskAdjust === true; }) &&
        calls.some(function (call) { return call.type === 'check12m' && call.mountId === 'intakeResults'; }) &&
        !!(skippedRisk && skippedRisk.didRun === false && skippedRisk.skippedReason === 'missing-post-render-risk-plan') &&
        stateChanged(before, after) === false &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
      ...
      samples: {
        ...
        committedKcal: root.intakeEstimatedKcalPerDay,
        riskCalls: calls.map(function (call) { return call.type; }),
        skippedReason: skippedRisk ? skippedRisk.skippedReason : null
      },
  ...
  function checkEstimatedIntakeInputModelModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaEstimatedIntakeInputModel');
    const snapshotFn = resolvePath('vildaGetEstimatedIntakeInputModelSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn({ executeSample: true }); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }

    const before = captureWindowState();
    const calls = [];
    const fakeDoc = { getElementById: function (id) { calls.push({ type: 'getElementById', id: id }); return id === 'intakePal' ? { value: '1.55' } : null; } };
    const sampleRows = [
      { ageYears: 6, ageMonths: 72, months: 72, weight: 21, height: 116 },
      { ageYears: 6.5, ageMonths: 78, months: 78, weight: 23, height: 119 }
    ];
    const deps = {
      document: fakeDoc,
      getUserBasics: function () { calls.push({ type: 'getUserBasics' }); return { sex: 'F', ageYears: 6.5, ageMonths: 78, weight: 23, height: 119 }; },
      readIntakeRows: function () { calls.push({ type: 'readIntakeRows' }); return sampleRows; },
      getIntakeRowHeight: function (row, fallbackHeight) { calls.push({ type: 'getIntakeRowHeight' }); return row && row.height ? row.height : fallbackHeight; },
      energyIsNumeric: function (value) { return Number.isFinite(Number(value)); },
      energyBuildIntakeObservedState: function (payload) { calls.push({ type: 'energyBuildIntakeObservedState', payload: payload }); return { teeRawKcal: 1200, teeBaselineKcal: 1320, modeBadge: { mode: 'smoke' } }; },
      buildIntakeIntervals: function () { return []; },
      getVildaEstimatedIntakeIntervalDependencies: function () { return { ENERGY_ADULT_START_AGE: 19, KCAL_PER_KG: 7700 }; },
      intakeUpdatePalDesc: function () { calls.push({ type: 'intakeUpdatePalDesc' }); }
    };
    const history = api && typeof api.buildEstimatedIntakeHistoryForRisk === 'function'
      ? api.buildEstimatedIntakeHistoryForRisk(sampleRows)
      : null;
    const input = api && typeof api.buildEstimatedIntakeCalcInputModel === 'function'
      ? api.buildEstimatedIntakeCalcInputModel({ updatePalDescription: true }, deps)
      : null;
    const observed = api && typeof api.buildEstimatedIntakeLastObservedModel === 'function'
      ? api.buildEstimatedIntakeLastObservedModel(input, {}, deps)
      : null;
    const calcDeps = api && typeof api.getEstimatedIntakeCalculationModelDependencies === 'function'
      ? api.getEstimatedIntakeCalculationModelDependencies({}, deps)
      : null;
    const after = captureWindowState();
    const contractDetail = checkNamedContracts(DEFAULT_ESTIMATED_INTAKE_INPUT_MODEL_CONTRACTS.slice(), { executeChecks: opts.executeContractChecks !== false, page: opts.page });

    return {
      ok: !!api &&
        api.VERSION === '1.0.0' &&
        api.STEP === '8Q-8' &&
        typeof api.buildEstimatedIntakeHistoryForRisk === 'function' &&
        typeof api.buildEstimatedIntakeCalcInputModel === 'function' &&
        typeof api.buildEstimatedIntakeLastObservedModel === 'function' &&
        typeof api.getEstimatedIntakeCalculationModelDependencies === 'function' &&
        typeof api.getSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.moduleOnly === true && snapshot.didRenderDom === false && snapshot.didWriteWindowState === false && snapshot.executedCalcEstimatedIntake === false) &&
        !!(history && history.length === 2 && history[0].ageMonths === 72 && history[0].weight === 21 && !Object.prototype.hasOwnProperty.call(history[0], 'height')) &&
        !!(input && input.step === '8Q-8' && input.kind === 'estimated-intake-input-model' && input.sex === 'F' && input.height === 119 && input.pal === 1.55 && input.rows.length === 2 && input.historyForRisk.length === 2) &&
        calls.some(function (call) { return call.type === 'intakeUpdatePalDesc'; }) &&
        !!(observed && observed.step === '8Q-8' && observed.kind === 'estimated-intake-last-observed-model' && observed.lastRow && observed.lastRow.ageMonths === 78 && observed.lastHeight === 119 && Math.abs(observed.teeFactor - 1.1) < 0.000001) &&
        !!(calcDeps && typeof calcDeps.buildIntakeIntervals === 'function' && typeof calcDeps.getIntakeRowHeight === 'function' && typeof calcDeps.energyBuildIntakeObservedState === 'function' && typeof calcDeps.energyIsNumeric === 'function' && calcDeps.intervalDependencies && calcDeps.intervalDependencies.KCAL_PER_KG === 7700) &&
        stateChanged(before, after) === false &&
        contractDetail.ok === true,
      version: api ? api.VERSION : null,
      step: api ? api.STEP : null,
      ...
      samples: {
        historyLength: history ? history.length : null,
        inputRows: input && Array.isArray(input.rows) ? input.rows.length : null,
        observedTeeFactor: observed ? observed.teeFactor : null,
        calcDeps: calcDeps ? Object.keys(calcDeps) : null
      },
      sideEffectsChangedOnGlobal: stateChanged(before, after),
      contractDetail
    };
  ...
  function checkEstimatedIntakeDomMountModule(options) {
    const opts = options || {};
    const api = resolvePath('VildaEstimatedIntakeDomMount');
    const snapshotFn = resolvePath('vildaGetEstimatedIntakeDomMountSnapshot') || (api && typeof api.getSnapshot === 'function' ? api.getSnapshot : null);
    let snapshot = null;
    if (typeof snapshotFn === 'function') {
      try { snapshot = snapshotFn(); } catch (error) { snapshot = { error: String(error && error.message ? error.message : error) }; }
    }
    ...
    const before = captureWindowState();
    const calls = [];
    const res = { innerHTML: '', textContent: '', className: 'intake-results', style: {} };
    const legendEl = { innerHTML: '', textContent: '', className: 'intake-legend', style: { display: 'initial' } };
    const fakeDoc = {
      getElementById: function (id) {
        calls.push({ type: 'getElementById', id: id });
        if (id === 'intakeResults') return res;
        if (id === 'intakeLegend') return legendEl;
        return null;
      }
    };
    const deps = {
      document: fakeDoc,
      vildaAppSetTrustedHtml: function (el, html, context) {
        calls.push({ type: 'setTrustedHtml', html: html, context: context });
        if (el) {
    ...
    const targets = api && typeof api.getEstimatedIntakeCalcOutputTargets === 'function'
      ? api.getEstimatedIntakeCalcOutputTargets({ document: fakeDoc })
      : null;
    const branchEmpty = api && typeof api.getEstimatedIntakeCalcBranch === 'function' ? api.getEstimatedIntakeCalcBranch({ rows: [] }, targets) : null;
    const branchSingle = api && typeof api.getEstimatedIntakeCalcBranch === 'function' ? api.getEstimatedIntakeCalcBranch({ rows: [{ weight: 20 }] }, targets) : null;
    const branchMulti = api && typeof api.getEstimatedIntakeCalcBranch === 'function' ? api.getEstimatedIntakeCalcBranch({ rows: [{ weight: 20 }, { weight: 21 }] }, targets) : null;
    const descBefore = api && typeof api.describeEstimatedIntakeCalcTargets === 'function' ? api.describeEstimatedIntakeCalcTargets(targets) : null;
  ...
  function checkUpdateHooksNutritionNormsWrapper(options) {
    ...
    return {
      ok: !!api &&
        ...
        typeof global.vildaGetNutritionNormsDirectRenderCoalescingSnapshot === 'function' &&
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.didCallWindowUpdate === false && snapshot.didRunHooks === false && snapshot.didPatchWindowUpdate === false) &&
        snapshot.migratedWrapperId === hookId &&
        !!(snapshot.afterUpdateCoalescing && snapshot.afterUpdateCoalescing.readOnly === true && snapshot.afterUpdateCoalescing.didCallWindowUpdate === false && snapshot.afterUpdateCoalescing.didRenderDom === false && snapshot.afterUpdateCoalescing.step === '8P-10') &&
        !!(snapshot.directRenderCoalescing && snapshot.directRenderCoalescing.readOnly === true && snapshot.directRenderCoalescing.didCallWindowUpdate === false && snapshot.directRenderCoalescing.didRenderDom === false && snapshot.directRenderCoalescing.step === '8P-11') &&
        snapshot.hookRegistered === true &&
        snapshot.legacyWrapperRemoved === true &&
      ...
      snapshot: snapshot ? clonePlain({
        ...
        afterUpdateCoalescing: snapshot.afterUpdateCoalescing,
        directRenderCoalescing: snapshot.directRenderCoalescing,
        registry: snapshot.registry,
  ...
  function checkUpdateHooksNutritionMicrosWrapper(options) {
    ...
    return {
      ok: !!api &&
        ...
        typeof global.vildaGetNutritionMicrosDirectRenderCoalescingSnapshot === 'function' &&
        typeof global.vildaGetNutritionMicrosProfileRenderCoalescingSnapshot === 'function' &&
        typeof global.vildaGetNutritionMicrosBootstrapRenderCoalescingSnapshot === 'function' &&
        typeof global.VildaNutritionMicros.getAfterUpdateCoalescingSnapshot === 'function' &&
        typeof global.VildaNutritionMicros.getDirectRenderCoalescingSnapshot === 'function' &&
        typeof global.VildaNutritionMicros.getProfileRenderCoalescingSnapshot === 'function' &&
        typeof global.VildaNutritionMicros.getBootstrapRenderCoalescingSnapshot === 'function' &&
        typeof global.nutritionMicrosRegisterAfterUpdateHook === 'function' &&
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.readOnly === true && snapshot.didCallWindowUpdate === false && snapshot.didRunHooks === false && snapshot.didPatchWindowUpdate === false) &&
        snapshot.migratedWrapperId === hookId &&
        !!(snapshot.afterUpdateCoalescing && snapshot.afterUpdateCoalescing.readOnly === true && snapshot.afterUpdateCoalescing.didCallWindowUpdate === false && snapshot.afterUpdateCoalescing.didRenderDom === false && snapshot.afterUpdateCoalescing.step === '8P-9') &&
        !!(snapshot.directRenderCoalescing && snapshot.directRenderCoalescing.readOnly === true && snapshot.directRenderCoalescing.didCallWindowUpdate === false && snapshot.directRenderCoalescing.didRenderDom === false && snapshot.directRenderCoalescing.step === '8P-12' && Array.isArray(snapshot.directRenderCoalescing.coalescedSources) && snapshot.directRenderCoalescing.coalescedSources.indexOf('nutritionMicrosDataReady') >= 0 && snapshot.directRenderCoalescing.coalescedSources.indexOf('vildaResultsModeChanged') >= 0) &&
        !!(snapshot.profileRenderCoalescing && snapshot.profileRenderCoalescing.readOnly === true && snapshot.profileRenderCoalescing.didCallWindowUpdate === false && snapshot.profileRenderCoalescing.didRenderDom === false && snapshot.profileRenderCoalescing.step === '8P-13' && Array.isArray(snapshot.profileRenderCoalescing.coalescedEventTypes) && snapshot.profileRenderCoalescing.coalescedEventTypes.indexOf('input') >= 0 && snapshot.profileRenderCoalescing.coalescedEventTypes.indexOf('change') >= 0 && Array.isArray(snapshot.profileRenderCoalescing.fieldIds) && snapshot.profileRenderCoalescing.fieldIds.indexOf('age') >= 0 && snapshot.profileRenderCoalescing.fieldIds.indexOf('height') >= 0 && snapshot.profileRenderCoalescing.fieldIds.indexOf('microsIronVariant') >= 0) &&
        !!(snapshot.bootstrapRenderCoalescing && snapshot.bootstrapRenderCoalescing.readOnly === true && snapshot.bootstrapRenderCoalescing.didCallWindowUpdate === false && snapshot.bootstrapRenderCoalescing.didRenderDom === false && snapshot.bootstrapRenderCoalescing.step === '8P-14' && Array.isArray(snapshot.bootstrapRenderCoalescing.delaysMs) && snapshot.bootstrapRenderCoalescing.delaysMs.join(',') === '0,60,180,500') &&
        snapshot.hookRegistered === true &&
        snapshot.legacyWrapperRemoved === true &&
  ...
  function checkGhTherapyIndexedDbResourceAudit(options) {
    ...
    return {
      ok: !!api &&
        ...
        typeof snapshotFn === 'function' &&
        !!(snapshot && snapshot.step === '8O-11l' && snapshot.readOnly === true && snapshot.openedIndexedDb === false && snapshot.queriedIndexedDb === false && snapshot.changedTherapyData === false && snapshot.postedBroadcastMessage === false && snapshot.performedFetch === false && snapshot.fetchedAsyncResource === false) &&
  ...
  function checkFetchTimeoutCleanup(options) {
    ...
    return {
      ok: Object.keys(helperChecks).every(function (key) { return helperChecks[key] === true; }) &&
        !!(snapshot && snapshot.step === '8O-11l' && snapshot.performedFetch === false && snapshot.fetchTimeoutCleanupApplied === true) &&
  ...
  function checkServiceWorkerFetchCacheStrategy(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.performedFetch === false && snapshot.serviceWorkerTimeoutExemptionAuditApplied === true && snapshot.serviceWorkerCacheStrategyAuditApplied === true && snapshot.serviceWorkerOfflineUpdateFlowSmokeApplied === true && snapshot.serviceWorkerOfflineUpdateFlowSmokeDefined === true && snapshot.serviceWorkerVersionedShellCacheKeyFixApplied === true && snapshot.serviceWorkerStaleCachePruningAuditApplied === true && snapshot.serviceWorkerRuntimeCacheGrowthRiskAudited === true && snapshot.scope && snapshot.scope.serviceWorkerRuntimeCachePruningImplemented === true) &&
  ...
  function checkServiceWorkerOfflineUpdateFlowSmoke(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.serviceWorkerOfflineUpdateFlowSmokeApplied === true && snapshot.serviceWorkerOfflineUpdateFlowSmokeDefined === true && snapshot.serviceWorkerVersionedShellCacheKeyFixApplied === true && snapshot.serviceWorkerStaleCachePruningAuditApplied === true && snapshot.serviceWorkerRuntimeCacheGrowthRiskAudited === true && snapshot.scope && snapshot.scope.serviceWorkerRuntimeCachePruningImplemented === true) &&
  ...
  function checkServiceWorkerVersionedShellCacheKey(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.serviceWorkerVersionedShellCacheKeyFixApplied === true && snapshot.serviceWorkerStaleCachePruningAuditApplied === true) &&
  ...
  function checkServiceWorkerStaleCachePruningAudit(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.serviceWorkerStaleCachePruningAuditApplied === true && snapshot.serviceWorkerRuntimeCacheGrowthRiskAudited === true && snapshot.scope && snapshot.scope.serviceWorkerRuntimeCachePruningImplemented === true) &&
  ...
  function checkServiceWorkerRuntimeCachePruning(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.cleanupApplied && snapshot.cleanupApplied.serviceWorkerRuntimeCachePruning === true) &&
  ...
  function checkServiceWorkerClientLifecycleAudit(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.cleanupApplied && snapshot.cleanupApplied.serviceWorkerClientLifecycle === true && snapshot.serviceWorkerClientLifecycleAuditApplied === true) &&
  ...
  function checkServiceWorkerClientUpdateUxSmoke(options) {
    ...
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.cleanupApplied && snapshot.cleanupApplied.serviceWorkerClientUpdateUxSmoke === true && snapshot.serviceWorkerClientUpdateUxSmokeApplied === true) &&
  ...
  function checkPwaManifestIconCacheAudit(options) {
    const opts = options || {};
    const audit = resolvePath('VildaGHTherapyResourceAudit');
    const snapshot = audit && typeof audit.getSnapshot === 'function' ? audit.getSnapshot({ includePlan: false }) : null;
    const serviceWorker = snapshot && snapshot.serviceWorker ? snapshot.serviceWorker : null;
    const pwa = serviceWorker && serviceWorker.pwaManifestIconCache ? serviceWorker.pwaManifestIconCache : null;
    const coverage = pwa && pwa.coverage ? pwa.coverage : null;
    const summary = snapshot && snapshot.summary ? snapshot.summary : null;
    const contractDetail = checkNamedContracts(['pwa-manifest-icon-cache-audit'], { executeChecks: opts.executeContractChecks !== false, page: opts.page });
    return {
      ok: !!(snapshot && snapshot.step === '8O-11l' && snapshot.cleanupApplied && snapshot.cleanupApplied.pwaManifestIconCache === true && snapshot.pwaManifestIconCacheAuditApplied === true) &&
        !!(serviceWorker && serviceWorker.pwaManifestIconCacheAuditApplied === true && serviceWorker.pwaManifestIconCacheRecommendedFollowUp === false) &&
        !!(pwa && pwa.startUrlScopeCovered === true && pwa.shortcutScopeCovered === true && pwa.iconFilesMaterializedCovered === true && pwa.htmlManifestLinksCovered === true && pwa.serviceWorkerIconCacheCovered === true && pwa.smokeCovered === true && pwa.smokeUsesMockedOnly === true && pwa.auditRegistersServiceWorker === false && pwa.auditTouchesCacheApi === false) &&
        !!(coverage && coverage.needsCleanup === false && coverage.startUrlScopeCovered === true && coverage.shortcutScopeCovered === true && coverage.iconFilesMaterializedCovered === true && coverage.htmlManifestLinksCovered === true && coverage.serviceWorkerIconCacheCovered === true && coverage.smokeCovered === true && coverage.smokeUsesMockedOnly === true) &&
        !!(summary && summary.pwaManifestIconCacheAuditComplete === true && summary.pwaManifestStartUrlScopeCovered === true && summary.pwaManifestShortcutScopeCovered === true && summary.pwaManifestIconFilesMaterializedCovered === true && summary.pwaManifestHtmlLinksCovered === true && summary.pwaManifestServiceWorkerIconCacheCovered === true && summary.pwaManifestIconCacheSmokeCovered === true && summary.pwaManifestIconCacheCleanupRecommended === false) &&
        contractDetail.ok === true,
      pwaManifestIconCache: pwa ? clonePlain(pwa, 5) : null,
      summary: summary ? clonePlain({
        pwaManifestIconCacheAuditComplete: summary.pwaManifestIconCacheAuditComplete,
        pwaManifestStartUrlScopeCovered: summary.pwaManifestStartUrlScopeCovered,
        pwaManifestShortcutScopeCovered: summary.pwaManifestShortcutScopeCovered,
        pwaManifestIconFilesMaterializedCovered: summary.pwaManifestIconFilesMaterializedCovered,
        pwaManifestHtmlLinksCovered: summary.pwaManifestHtmlLinksCovered,
        pwaManifestServiceWorkerIconCacheCovered: summary.pwaManifestServiceWorkerIconCacheCovered,
        pwaManifestIconCacheSmokeCovered: summary.pwaManifestIconCacheSmokeCovered,
        pwaManifestIconCacheCleanupRecommended: summary.pwaManifestIconCacheCleanupRecommended
      }, 3) : null,
      contractDetail
    };
  }
  ...
  function runRegressionSuite(options) {
    ...
    tests.push(runCase('estimated-intake-ui', 'estimated-intake-ui', true, checkEstimatedIntakeUiModule, opts));
    tests.push(runCase('estimated-intake-runtime', 'estimated-intake-runtime', true, checkEstimatedIntakeRuntimeModule, opts));
    tests.push(runCase('estimated-intake-input-model', 'estimated-intake-input-model', true, checkEstimatedIntakeInputModelModule, opts));
    tests.push(runCase('estimated-intake-dom-mount', 'estimated-intake-dom-mount', true, checkEstimatedIntakeDomMountModule, opts));
    ...
    tests.push(runCase('growth-reference-data-module', 'growth-reference-data', true, checkGrowthReferenceDataModule, opts));
    tests.push(runCase('professional-module', 'professional-module', true, checkProfessionalModule, opts));
    tests.push(runCase('persist-runtime', 'persist-runtime', true, checkPersistRuntime, opts));
    tests.push(runCase('summary-cards', 'summary-cards', true, checkSummaryCardsModule, opts));
    tests.push(runCase('diet-plan-ui', 'diet-plan-ui', true, checkDietPlanUiModule, opts));
    ...
    tests.push(runCase('pwa-manifest-icon-cache-audit', 'pwa-cache', true, checkPwaManifestIconCacheAudit, opts));
  ...
  function getSnapshot(options) {
    ...
    const snapshot = {
      ...
      growthReferenceData: {
        module: hasObject('VildaGrowthReferenceData'),
        snapshotApi: hasFunction('vildaGetGrowthReferenceDataSnapshot'),
        apiSurface: hasObject('VildaGrowthReferenceData') ? {
          version: resolvePath('VildaGrowthReferenceData.VERSION') || null,
          step: resolvePath('VildaGrowthReferenceData.STEP') || null,
          getData: hasFunction('VildaGrowthReferenceData.getData'),
          getSnapshot: hasFunction('VildaGrowthReferenceData.getSnapshot'),
          WFL_DATA_GIRLS: Array.isArray(resolvePath('VildaGrowthReferenceData.WFL_DATA_GIRLS')),
          WFL_DATA_BOYS: Array.isArray(resolvePath('VildaGrowthReferenceData.WFL_DATA_BOYS')),
          bmiPercentiles: hasObject('VildaGrowthReferenceData.bmiPercentiles'),
          LMS_BOYS: hasObject('VildaGrowthReferenceData.LMS_BOYS'),
          LMS_GIRLS: hasObject('VildaGrowthReferenceData.LMS_GIRLS'),
          LMS_INFANT_BOYS: hasObject('VildaGrowthReferenceData.LMS_INFANT_BOYS'),
          LMS_INFANT_GIRLS: hasObject('VildaGrowthReferenceData.LMS_INFANT_GIRLS'),
          bmiInfantBoys: hasObject('VildaGrowthReferenceData.bmiInfantBoys'),
          bmiInfantGirls: hasObject('VildaGrowthReferenceData.bmiInfantGirls')
        } : null
      },
      professionalModule: {
        module: hasObject('VildaProfessionalModule'),
        snapshotApi: hasFunction('vildaGetProfessionalModuleSnapshot'),
        apiSurface: hasObject('VildaProfessionalModule') ? {
          version: resolvePath('VildaProfessionalModule.VERSION') || null,
          step: resolvePath('VildaProfessionalModule.STEP') || null,
          init: hasFunction('VildaProfessionalModule.init'),
          initProfessionalModule: hasFunction('VildaProfessionalModule.initProfessionalModule'),
          getSnapshot: hasFunction('VildaProfessionalModule.getSnapshot')
        } : null
      },
      persistRuntime: {
        module: hasObject('VildaPersistRuntime'),
        snapshotApi: hasFunction('vildaGetPersistRuntimeSnapshot'),
        coalescingSnapshotApi: hasFunction('vildaGetPersistAutosaveCoalescingSnapshot'),
        apiSurface: hasObject('VildaPersistRuntime') ? {
          version: resolvePath('VildaPersistRuntime.VERSION') || null,
          step: resolvePath('VildaPersistRuntime.STEP') || null,
          init: hasFunction('VildaPersistRuntime.init'),
          initPersistRuntime: hasFunction('VildaPersistRuntime.initPersistRuntime'),
          getSnapshot: hasFunction('VildaPersistRuntime.getSnapshot')
        } : null
      },
      summaryCards: {
        module: hasObject('VildaSummaryCards'),
        snapshotApi: hasFunction('vildaGetSummaryCardsSnapshot'),
        apiSurface: hasObject('VildaSummaryCards') ? {
          version: resolvePath('VildaSummaryCards.VERSION') || null,
          step: resolvePath('VildaSummaryCards.STEP') || null,
          init: hasFunction('VildaSummaryCards.init'),
          initSummaryCards: hasFunction('VildaSummaryCards.initSummaryCards'),
          getSnapshot: hasFunction('VildaSummaryCards.getSnapshot'),
          generateMetabolicSummary: hasFunction('generateMetabolicSummary'),
          handleMetabolicSummaryClick: hasFunction('handleMetabolicSummaryClick')
        } : null
      },
      estimatedIntake: {
        ...
        diagnosticApis: {
          ...
          ui: hasFunction('vildaGetEstimatedIntakeUiSnapshot'),
          runtime: hasFunction('vildaGetEstimatedIntakeRuntimeSnapshot')
        },
        ui: {
          module: hasObject('VildaEstimatedIntakeUI'),
          version: resolvePath('VildaEstimatedIntakeUI.VERSION') || null,
          step: resolvePath('VildaEstimatedIntakeUI.STEP') || null,
          buildResultsHtml: hasFunction('VildaEstimatedIntakeUI.buildResultsHtml')
        },
        runtime: {
          module: hasObject('VildaEstimatedIntakeRuntime'),
          version: resolvePath('VildaEstimatedIntakeRuntime.VERSION') || null,
          step: resolvePath('VildaEstimatedIntakeRuntime.STEP') || null,
          commitCalcModel: hasFunction('VildaEstimatedIntakeRuntime.commitEstimatedIntakeCalcModelWindowState'),
          runPostRenderRisk: hasFunction('VildaEstimatedIntakeRuntime.runEstimatedIntakePostRenderRisk')
        }
      },
      deps: {
        ...
        growthReferenceDataContract: getContractNames().indexOf('growth-reference-data') !== -1,
        professionalModuleContract: getContractNames().indexOf('professional-module') !== -1,
        persistRuntimeContract: getContractNames().indexOf('persist-runtime') !== -1,
        summaryCardsContract: getContractNames().indexOf('summary-cards') !== -1,
        dietPlanUiContract: getContractNames().indexOf('diet-plan-ui') !== -1,
        estimatedIntakeUiContract: getContractNames().indexOf('estimated-intake-ui') !== -1,
        estimatedIntakeRuntimeContract: getContractNames().indexOf('estimated-intake-runtime') !== -1,
        ...
        pwaManifestIconCacheAuditContract: getContractNames().indexOf('pwa-manifest-icon-cache-audit') !== -1
      },
      ...
      ghTherapyResources: {
        ...
        apiSurface: hasObject('VildaGHTherapyResourceAudit') ? {
          ...
          getPwaManifestIconCacheSnapshot: hasFunction('VildaGHTherapyResourceAudit.getPwaManifestIconCacheSnapshot')
        } : null
      },
      fetchTimeoutCleanup: {
        helpers: {
