  ...
  function initUserData() {
    ...
    fields.forEach(({ id, key }) => {
      ...
      if (key === 'sex') {
        ...
        // Bug 1 fix vol2 (2026-05): blokujemy pole płeć TYLKO gdy mamy
        // faktycznie załadowanego pacjenta — sygnałem jest niepuste imię
        // w storage (`stored.name` / `stored.nameLocked`). Sex ma default
        // "M" więc ocenianie po samym storedValue zawsze przepuszczało.
        const hasLoadedPatient = !!(stored.name || stored.nameLocked);
        if (stored.sexLocked && hasLoadedPatient) {
          try { el.disabled = true; } catch (error) { logUserDataWarn('Zignorowany błąd pomocniczy w synchronizacji danych użytkownika', error); }
        } else {
          try { el.disabled = false; } catch (error) { logUserDataWarn('Zignorowany błąd pomocniczy w synchronizacji danych użytkownika', error); }
        }
