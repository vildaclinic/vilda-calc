...
if (typeof workbox !== 'undefined') {
  ...
  // Set a custom cache name prefix to avoid collisions with other sites. Use a
  // unique prefix for this specific application (steroid dose calculator). When
  // creating additional apps, update the prefix accordingly in their respective
  ...
  workbox.precaching.precacheAndRoute([
    ...
    // dose calculator, we precache its main HTML page and manifest. When
    // creating additional apps, adjust these filenames and revision values
    // accordingly.
    { url: 'kalkulator_kortykosteroidow.html', revision: '9f14a270549cc33d239e86cb6684d4fc' },
    { url: 'manifest.json', revision: '63e712f665366b0613b8479a7a4212c8' },
    ...
    // Note: Icons are not precached here because they were not provided in the
    // project files. They will still be cached at runtime via the static
  ...
  const CACHE_NAME = 'pwa-steroids-v1';
  ...
  // handler below.
  const urlsToCache = [
    // Cache the main page of this app for offline use. For additional
    // applications, replace this entry with the appropriate HTML filename.
    'kalkulator_kortykosteroidow.html',
    'manifest.json',
    'service-worker.js'
  ];
        ...
        return fetch(event.request)
          ...
          .catch(() => {
            if (event.request.mode === 'navigate') {
              // When navigating offline, serve the cached main page of this app.
              // Update this filename when copying the service worker to other apps.
              return caches.match('kalkulator_kortykosteroidow.html');
            }
          });
      })
    );
