...
test('nastolatek z otyłością: pierwszy szczebel drabinki w akapicie o masie; rówieśnik tylko z opcji, osobnym akapitem', async ({ page }) => {
  ...
  const oczek = 'Obecna masa ciała wynosi 88,0 kg. Pierwszy cel to ok. ' + przec(sz.masa, 1) + ' kg, czyli około ' + przec(88 - sz.masa, 1)
    // rata U: korzyść Reinehra tylko pod progiem Reinehra; pod innym szczeblem (tu: wyjście z otyłości olbrzymiej) opis w nawiasie
    + (sz.klucz === 'reinehr' ? ' kg mniej; już taka zmiana poprawia ciśnienie i wyniki badań krwi (cholesterol, trójglicerydy).' : ' kg mniej (' + sz.opis + ').')
    + ' Górna granica normy dla wieku i wzrostu odpowiada masie ok. '
...
test('młodsze dziecko: „Waga dziecka” z pierwszym celem, bez rówieśnika; stabilizacja bez rówieśnika', async ({ page }) => {
  ...
  if (sz && sz.masa < 45 && sz.masa > r.masa.gornaNormaKg) {
    expect(t).toContain('Waga dziecka: 45,0 kg. Pierwszy cel to ok. ' + przec(sz.masa, 1) + ' kg, czyli około ' + przec(45 - sz.masa, 1)
      + (sz.klucz === 'reinehr' ? ' kg mniej; już taka zmiana poprawia ciśnienie i wyniki badań krwi (cholesterol, trójglicerydy).' : ' kg mniej (' + sz.opis + ').') + ' Aby masa ciała znalazła się w górnej granicy normy dla wieku i wzrostu, powinna wynosić ok. '
