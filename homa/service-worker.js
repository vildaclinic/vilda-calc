...
if (typeof workbox !== 'undefined') {
  ...
  workbox.precaching.precacheAndRoute([
    ...
    // latest version from the network and update the cache.  For this app
    // (HOMA‑IR calculator), we cache its main HTML file and manifest. When
    // creating additional apps, adjust these filenames and revision values as
    // appropriate for each directory.
    { url: 'HOMA_2.html', revision: '0d4853c0fde9b959e5163ffacd41d91f' },
    { url: 'manifest.json', revision: '397a47309ccdc1a5b498700999f26b2a' },
  ...
  // handler below.
  const urlsToCache = [
    // Cache the main page of this app so it is available offline. For
    // additional apps, replace this with the appropriate HTML file name.
    'HOMA_2.html',
    'manifest.json',
    'service-worker.js'
  ];
        ...
        return fetch(event.request)
          ...
          .catch(() => {
            if (event.request.mode === 'navigate') {
              // When navigating offline, serve the cached main page of this
              // app. Update the filename here to match the HTML file of
              // additional apps when creating separate service workers.
              return caches.match('HOMA_2.html');
            }
          });
      })
    );
