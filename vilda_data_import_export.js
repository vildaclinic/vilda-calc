/*
 * Vilda Data Import/Export Helper v1.10.3
 *
...
(function (global) {
  ...
  function saveUserData(options) {
    ...
    try {
      ...
      const blob = new global.Blob([jsonStr], { type: 'application/json' });
      ...
      a.click();
      setTimeout(() => {
        try { urlApi.revokeObjectURL(url); } catch (error) { logSwallowed('vilda_data_import_export:revokeObjectURL', error); }
        try { if (a.parentNode) a.parentNode.removeChild(a); } catch (error) { logSwallowed('vilda_data_import_export:removeDownloadAnchor', error); }
      }, 0);
      return data;
    } catch (error) {
      logSwallowed('vilda_data_import_export:saveUserData', error);
      callTooltip(saveBtnEl, 'Nie udało się zapisać danych do pliku JSON.', opts);
      return null;
    }
  }
  ...
  function applyLoadedData(data, options){
    ...
    if (data.clcr && typeof data.clcr === 'object') {
      ...
      try {
        ...
        if (form && data.clcr.inputs && typeof data.clcr.inputs === 'object') {
          ...
          if (typeof global.update === 'function') safeCall('clcr-update-after-import', global.update, []);
        }
      } catch (error) {
    ...
    if (typeof global.vildaPersistScheduleSave === 'function') {
      try {
    ...
    if (debouncedUpdateFn) safeCall('debounced-update', debouncedUpdateFn, []);
