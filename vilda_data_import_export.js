...
(function (global) {
  ...
  function applyLoadedDataToSharedUserRoot(root, persistRoot, data, name) {
    ...
    if (data && data.user && typeof data.user === 'object') {
      ...
      if (data.user.sex) {
        ...
        // Bug 1 fix vol3 (2026-05): zapisujemy `sexLocked: true` w shared
        // user data TYLKO gdy mamy faktycznego pacjenta (`name` niepuste).
        // Bez tego autosave głównej sesji ze sex='M' (default) bez imienia
        // zapisywał `sexLocked: true` w shared, co blokowało pole sex
        // przy każdej kolejnej nawigacji (m.in. wejściu na klirens).
        if (name) {
          shared.sexLocked = true;
        } else if (shared.sexLocked) {
          delete shared.sexLocked;
        }
  ...
  function applyLoadedData(data, options){
    ...
    if (!isClcrJsonImportPreviewOnly && data.user && data.user.sex && q('sex')) {
      ...
      // Bug 1 fix vol3 (2026-05): blokujemy sex TYLKO gdy mamy faktycznego
      // pacjenta (sygnał: niepusta nazwa). Default `<option value="M">` w
      // selektorze sex powodował, że autosave głównej sesji ze sex='M' (bez
      // imienia) traktowany był jako "loaded patient" → blokada sex przy
      // każdym restore na innych podstronach (m.in. klirens).
      if (name) {
        try { sexEl.disabled = true; } catch (error) { logSwallowed('vilda_data_import_export:applyLoadedData:sex-disabled', error); }
      } else {
        try { sexEl.disabled = false; } catch (error) { logSwallowed('vilda_data_import_export:applyLoadedData:sex-undisabled', error); }
      }
    }
