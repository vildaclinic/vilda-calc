...
// Faza 2 / krok 9-10: split na CRITICAL_SHELL i BACKGROUND_SHELL.
//
// CRITICAL_SHELL_URLS — absolutne minimum, bez którego aplikacja nie odpali offline.
// Pobierane sekwencyjnie podczas install, REQUIRED. Jeśli któryś plik się nie pobierze,
// install fail i stara wersja SW zostaje aktywna.
// To jest ten warunek "nowa wersja nie zastępuje starej, gdy install fail".
//
// BACKGROUND_SHELL_URLS — pozostałe pliki shellu. Pobierane RÓWNOLEGLE po activate
// (event 'activate' już wywołał claim, więc aplikacja działa). Jeśli którykolwiek
// się nie pobierze, dociąga go runtime cache (cache-first → miss → network → cache).
// To znacząco przyspiesza instalację nowej wersji SW (~85 plików sekwencyjnie ≫
// równoległe ~75 plików w tle, bez blokowania activate).
//
// Krok 10: usunąłem zduplikowane wpisy `/x.js` + `/x.js?v=N`. HTML zawsze referuje
// wersjonowaną nazwę, więc niewersjonowana w shell-cache była redundantna.
const CRITICAL_SHELL_URLS = [
  ...
  '/logo_vilda.jpeg',
  '/vilda_logger.js?v=3',
  '/vilda_navigator.js?v=1',
  '/vilda_html.js?v=4',
  '/vilda_persistence_adapter.js?v=7',
  '/vilda_init.js?v=4',
  '/vilda_deps.js?v=75',
  '/vilda_app_helpers.js?v=2',
  '/data/lms-tables.js?v=1',
  '/app.js?v=152'
];

const BACKGROUND_SHELL_URLS = [
  ...
  '/nutrition_micros.js?v=21',
  '/data/micronorms-fallback.js?v=1',
  '/adult_vitals.js?v=5',
  '/data/ds_lms.js?v=1',
  '/vilda_down_syndrome.js?v=1',
  '/vilda_anorexia_risk.js?v=1',
  '/data/centile_data.js?v=1',
  '/data/bayley_pinneau_data.js?v=1',
  '/data/rwt_data.js?v=1',
  '/data/reinehr_cdgp_data.js?v=1',
  '/data/advanced_growth_kowd.js?v=1',
  '/vitalSigns.js',
  '/gh_igf_therapy.js?v=7',
  '/antibiotic_therapy.js?v=11',
  '/data/antibiotic-tables.js?v=1',
  '/userData.js?v=6',
  '/ios26-ui.js?v=19',
  '/tutorial.js?v=5',
  '/bp_module.js?v=4',
  ...
  '/data/sga_intergrowth_data.js?v=1',
  '/custom-fixes.js?v=12',
  '/reposition.js?v=5',
  '/growth-basic-module.js?v=8'
];

// Wsteczna kompatybilność dla istniejącej logiki (PRECACHE_URLS, fetch handler) —
// CORE_SHELL_URLS pozostaje aliasem zawierającym CRITICAL + BACKGROUND, żeby reszta
// pliku (np. handler nawigacji, cache-key resolver) nie wymagała przeróbek.
const CORE_SHELL_URLS = [...CRITICAL_SHELL_URLS, ...BACKGROUND_SHELL_URLS];
...
async function installShell() {
  // Faza 2 / krok 9: install ogranicza się do CRITICAL_SHELL_URLS.
  // Sekwencyjnie + required: jeśli któryś plik nie zejdzie, install fail
  // i stara wersja SW pozostaje aktywna (zachowanie wymagane dla bezpiecznych deploy-ów).
  for (const url of CRITICAL_SHELL_URLS) {
    await fetchAndStorePrecacheUrl(url, { required: true });
  }
}

// Faza 2 / krok 9: precache w tle z ograniczoną liczbą jednoczesnych requestów.
// Wywoływany po activate, gdy SW przejmie kontrolę nad klientem. Jeśli któryś plik
// nie zejdzie, fetch handler (cache-first → runtime) sam go potem dociągnie.
async function precacheInBackground(urls, concurrency = 6) {
  let i = 0;
  async function worker() {
    while (i < urls.length) {
      const idx = i++;
      try {
        await fetchAndStorePrecacheUrl(urls[idx], { required: false });
      } catch (_) {
        void _;
      }
    }
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    ...
    void _;
      ...
      // Faza 2 / krok 9: po claim dociągamy resztę shella w tle, równolegle.
      // Nie awaitujemy — fire-and-forget, żeby activate kończyło się natychmiast.
      // event.waitUntil też się tu zakończy, ale background precache już biegnie.
      precacheInBackground([...BACKGROUND_SHELL_URLS, ...OPTIONAL_DOCUMENTS, ...OPTIONAL_ASSETS], 6);
    })()
  );
});
