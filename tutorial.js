...
(() => {
  ...
  function observeDesktopDockAwareOffset() {
    ...
    document.addEventListener('scroll', scheduleFast, { passive: true, capture: true });
  ...
  function init() {
    ...
    // Turbo cleanup: kiedy użytkownik klika link, Turbo Drive odpala
    // `turbo:before-visit`. Jeśli tutorial jest otwarty, jego overlay
    // przyklejony do <body> i tak zostanie zniszczony razem ze starą stroną,
    // ale state.overlay zostaje w closure i wskazuje na detachowany element,
    // przez co na nowej stronie boot() pominie tworzenie świeżego overlay
    // (bo `if (state.overlay) return;`). Zamykamy tutorial i zerujemy
    // state, żeby boot() na docelowej stronie zaczął od czystej kartki.
    if (typeof document.addEventListener === 'function') {
      document.addEventListener('turbo:before-visit', function () {
        try {
          if (state && state.overlay) {
            // Schowanie sheetu emituje toasta o launcher hint — na Turbo-nawi
            // tego nie chcemy, więc usuwamy DOM bezpośrednio i czyścimy state.
            try { state.overlay.classList.remove('is-open'); } catch (_) {}
            try {
