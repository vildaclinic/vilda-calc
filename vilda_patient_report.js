...
function updateProfessionalSummaryCard(_retry) {
  ...
  // FIX (2026-06-03): sam MPH liczy się z ZAPISANYCH wzrostów rodziców nawet przy
  // pustym formularzu (np. tuż po „Wczytaj pacjenta → Nowy pomiar" albo po
  // przeładowaniu strony) — karta pokazywała się tylko z linią MPH (a po reloadzie
  // dodatkowo z przyciskiem „Raport PDF"). MPH bez realnego pomiaru to nie wynik:
  // kartę pokazujemy dopiero, gdy jest ≥1 linia NIE-MPH; wtedy MPH dołącza normalnie.
  const hasNonMphLine = Array.isArray(lines) && lines.some(function (l) {
    return !/^\s*MPH\b/i.test(String(l || ''));
  });
  const shouldShow = hasNonMphLine;
