...
(function setupPatientReportPdfButton() {
  if (typeof document === 'undefined') return;
  // Anti-leak (Etap 4.2): instalujemy raz przez vildaInstallGlobalListener.
  // Bez tego każda re-evaluacja IIFE (Turbo edge cases) dodawała kolejny
  // delegowany click handler — przy 5 nawigacjach każdy klik na przycisk PDF
  // otwierał 5 dialogów.
  function patientReportPdfClickHandler(event) {
  ...
  if (typeof window !== 'undefined' && typeof window.vildaInstallGlobalListener === 'function') {
    window.vildaInstallGlobalListener(
      'vilda-patient-report:pdf-button-delegate',
      document,
      'click',
      patientReportPdfClickHandler
    );
  } else {
    document.addEventListener('click', patientReportPdfClickHandler);
  }
})();
