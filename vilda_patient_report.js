...
function patientReportMasaWobecBmi(nota,dorosly,centylMasy,modelBmi){try{if(dorosly)return nota;var Ms=typeof window<"u"?window.VildaMasa:null;if(!Ms||typeof Ms.wyjasnienieWzgledemBmi!="function")return nota;var w=Ms.wyjasnienieWzgledemBmi({centylMasy:centylMasy,kategoriaBmi:modelBmi&&modelBmi.kategoria||null});return w&&w.rozne&&w.zdanie?patientReportAppendSentence(nota,w.zdanie):nota}catch(e){return nota}}
...
function patientReportPierwszyKrok(drab,masaKg){if(!drab||drab.kierunek!=="redukcja"||!drab.cel)return null;const pierwszy=drab.szczeble&&drab.szczeble.length?drab.szczeble[0]:drab.cel,m=Number(masaKg),pm=Number(pierwszy&&pierwszy.masa);if(!isFinite(m)||!isFinite(pm)||!(m-pm>0))return null;const jestSzczebel=!!(drab.szczeble&&drab.szczeble.length),reinehr=pierwszy.klucz==="reinehr";return{masaKg:pm,roznicaKg:m-pm,bmi:isFinite(Number(pierwszy.bmi))?Number(pierwszy.bmi):null,klucz:pierwszy.klucz||null,opis:reinehr?"":String(pierwszy.opis||pierwszy.etykieta||""),jestSzczebel,/* korzysc zdrowotna: u dziecka tylko prog Reinehra (−0,5 BMI-SDS), u doroslego kazdy szczebel — jak zdanie A generatora (rata O) */korzysc:jestSzczebel&&(!!drab.dorosly||reinehr)}}
const PATIENT_REPORT_KORZYSC_KROKU="już ta zmiana poprawia ciśnienie i wyniki badań krwi";
function patientReportZdaniePierwszegoKroku(krok){if(!krok)return"";const masa=patientReportFormatNumber(krok.masaKg,1)+" kg",roz=patientReportFormatNumber(krok.roznicaKg,1)+" kg",nawias=krok.opis?" ("+krok.opis+")":"";return krok.jestSzczebel?`Pierwszy krok to ok. ${masa}${nawias}, czyli około ${roz} mniej`+(krok.korzysc?`; ${PATIENT_REPORT_KORZYSC_KROKU}.`:"."):`Cel to ok. ${masa}${nawias}, czyli około ${roz} mniej.`}
...
function patientReportAdultBmiRangeReference(bmi,delta){const dol=ADULT_BMI.UNDER,gora=24.9,label="Zakres prawidłowy BMI dla dorosłych",zakres=patientReportFormatNumber(dol,1)+"–"+patientReportFormatNumber(gora,1);if(typeof bmi!="number"||!isFinite(bmi))return{available:!1,label,medianText:"Brak por\xF3wnania do zakresu prawidłowego.",diffText:"",neutral:!0};const stan=delta&&delta.state;return bmi>gora+.049||stan==="above-normal"?{available:!0,label,medianText:zakres,diffText:"To o "+patientReportFormatNumber(bmi-gora,1)+" pkt powyżej g\xF3rnej granicy.",neutral:!1}:bmi<dol-.049||stan==="underweight"?{available:!0,label,medianText:zakres,diffText:"To o "+patientReportFormatNumber(dol-bmi,1)+" pkt poniżej dolnej granicy.",neutral:!1}:{available:!0,label,medianText:zakres,diffText:"BMI mieści się w tym zakresie.",neutral:!0}}
/* ---- data pomiaru ---- */
...
function patientReportBuildEnergyCardContext(dane,b,drab){const out={celEnergiaKcal:null,celMasaKg:null,celWlasny:!1,bialko:null,weglProc:null,tluszczProc:null};if(!dane||!dane.energia||!b)return out;const en=dane.energia,m=dane.masa||{},kl=dane.klasyfikacja||{},mies=typeof document<"u"?parseFloat(document.getElementById("ageMonths")?.value||"0"):0,pozaNorma=!!(kl.nadmiar||kl.niedowaga),/* generator nie oddaje masy docelowej dziecka z niedowaga — wtedy cel z drabinki (ta sama granica, ktora pokazuje karta masy) */celDrab=pozaNorma&&drab&&drab.cel&&isFinite(Number(drab.cel.masa))?Number(drab.cel.masa):NaN,cel=isFinite(Number(m.docelowaKg))&&Number(m.docelowaKg)>0?Number(m.docelowaKg):celDrab;out.celWlasny=String(dane.strategia||"")==="cel-wlasny";if(isFinite(cel)&&cel>0&&typeof window<"u"&&typeof window.energyBuildPlanReductionState=="function")try{const st=window.energyBuildPlanReductionState({ageYears:b.ageYears,ageMonthsOpt:isFinite(mies)?mies:0,sex:b.sex,weightKg:cel,heightCm:b.height,palInput:en.palUzyty,history:null,intakeKcalPerDay:null,customGoalKg:null,growthEnded:!1}),/* teeRaw: REE×PAL×wzrastanie bez korekty ryzyka (korekta ZO dotyczy planu, nie zapotrzebowania dla masy docelowej) */tee=st&&Number(isFinite(Number(st.teeRawKcal))&&Number(st.teeRawKcal)>0?st.teeRawKcal:st.teeBaselineKcal);isFinite(tee)&&tee>0&&(out.celEnergiaKcal=tee,out.celMasaKg=cel)}catch{}if(typeof window<"u"&&typeof window.nutritionNormsBuildCardModel=="function")try{const nm=window.nutritionNormsBuildCardModel({ageYears:b.ageYears,ageMonthsOpt:isFinite(mies)?mies:0,sex:b.sex,weightKg:b.weight,heightCm:b.height,mainPal:en.palUzyty},window.nutritionNormsUiState&&typeof window.nutritionNormsUiState=="object"?window.nutritionNormsUiState:{});out.weglProc=nm&&nm.carbs&&Array.isArray(nm.carbs.percentRange)?nm.carbs.percentRange:null,out.tluszczProc=nm&&nm.fat&&Array.isArray(nm.fat.percentRange)?nm.fat.percentRange:null;const t=nm&&nm.protein&&nm.protein.targets;if(t&&t.available&&isFinite(Number(t.rda_g_per_kg))){const rda=Number(t.rda_g_per_kg);let masaRef=Number(t.referenceWeightKg),podstawa="masa referencyjna dla wieku wg Norm 2024",podstawaKrotka="masa referencyjna";isFinite(masaRef)&&masaRef>0||(pozaNorma&&isFinite(cel)&&cel>0?(masaRef=cel,podstawa="masa prawidłowa",podstawaKrotka="masa prawidłowa"):(masaRef=Number(b.weight),podstawa="obecna masa",podstawaKrotka="obecna masa")),isFinite(masaRef)&&masaRef>0&&(out.bialko={rdaGKg:rda,masaRefKg:masaRef,podstawa,podstawaKrotka})}}catch{}return out}
function patientReportBuildEnergyCardFromData(dane,ctx){const c=ctx||{},brak={kind:"energy-demand",title:"Zapotrzebowanie energetyczne",subtitle:"Energia i makroskładniki",badge:"Brak danych",value:"—",note:"Brak kompletu danych (wiek, płeć, masa ciała, wzrost) do obliczenia zapotrzebowania energetycznego.",rows:[],tableHeaders:["Pozycja","Wartość"]};if(!dane||!dane.energia)return brak;const en=dane.energia,kl=dane.klasyfikacja||{},L=v=>{const n=Number(v);return v==null||!isFinite(n)?null:n},kcal=v=>patientReportFormatNumber(Math.round(v),0)+" kcal/d",wNormie=!kl.nadmiar&&!kl.niedowaga,obecna=L(en.utrzymanieKcal)!=null?L(en.utrzymanieKcal):L(en.teeBazowyKcal),rows=[];wNormie&&obecna!=null&&rows.push({label:"Przy obecnej masie",valueText:kcal(obecna)}),c.celEnergiaKcal!=null&&rows.push({label:(c.celWlasny?"Dla masy docelowej (cel własny, ":"Dla masy prawidłowej (")+patientReportFormatNumber(c.celMasaKg,1)+" kg)",valueText:kcal(c.celEnergiaKcal)});let plan="";Array.isArray(en.podazZakresKcal)&&L(en.podazZakresKcal[0])!=null&&L(en.podazZakresKcal[1])!=null?plan=patientReportFormatNumber(Math.round(en.podazZakresKcal[0]),0)+"–"+patientReportFormatNumber(Math.round(en.podazZakresKcal[1]),0)+" kcal/d":L(en.podazZaokrKcal)!=null&&(plan=kcal(en.podazZaokrKcal)),plan&&rows.push({label:"Plan"+(en.dietaNazwa?": dieta "+String(en.dietaNazwa).toLowerCase():": zalecana kaloryczność"),valueText:plan,highlighted:!0}),c.bialko&&rows.push({label:"Białko",valueText:patientReportFormatNumber(c.bialko.rdaGKg,2)+" g/kg \xD7 "+patientReportFormatNumber(c.bialko.masaRefKg,0)+" kg ("+(c.bialko.podstawaKrotka||c.bialko.podstawa||"")+") \u2248 "+patientReportFormatNumber(Math.round(c.bialko.rdaGKg*c.bialko.masaRefKg),0)+" g/d"});const proc=z=>Array.isArray(z)&&L(z[0])!=null&&L(z[1])!=null?patientReportFormatNumber(z[0],0)+"–"+patientReportFormatNumber(z[1],0)+"\xA0% energii":"";proc(c.weglProc)&&rows.push({label:"Węglowodany",valueText:proc(c.weglProc)}),proc(c.tluszczProc)&&rows.push({label:"Tłuszcze",valueText:proc(c.tluszczProc)});if(!rows.length)return brak;/* rata R (decyzja 7): bez noty o wzorze i PAL — odznaka nazywa poziom aktywnosci slowami, podstawa bialka w wierszu */const akt=PATIENT_REPORT_PAL_OPISY[String(Number(en.palUzyty))]||"",value=plan||(c.celEnergiaKcal!=null?kcal(c.celEnergiaKcal):obecna!=null?kcal(obecna):"\u2014");return{kind:"energy-demand",title:"Zapotrzebowanie energetyczne",subtitle:"Energia i makrosk\u0142adniki",badge:akt||"Informacyjnie",value,note:"",rows,tableHeaders:["Pozycja","Warto\u015B\u0107"]}}
