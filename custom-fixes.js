(function (global) {
  ...
  // ============ vildaOnReady: rejestr inicjalizatorów per-page-render ============
  // Etap pre-Turbo (2025-05): rejestrujemy każdy `vildaOnReady('name', fn)` i
  // odpalamy go:
  //   1) raz po pierwszym DOMContentLoaded (zachowanie backward-compat),
  //   2) raz po każdym `turbo:load` (kiedy Turbo Drive zostanie zainstalowany).
  //
  // Rejestracje nowych callbacków po DOMContentLoaded uruchamiają je natychmiast,
  // tak jak wcześniej. Dzięki temu — bez zainstalowanego Turbo — nic się nie
  // zmienia funkcjonalnie. Po dodaniu Turbo Etap 1 automatycznie zaczyna re-initować
  // wszystkie zarejestrowane moduły przy każdej nawigacji bez pełnego reloadu.
  //
  // Opcje:
  //   { turboReload: false } — moduł jest jednorazowy, nie re-initować przy nav
  //     (default: true — re-init przy każdej Turbo-nawigacji).
  //   { once: false }        — przekazywana do vildaSafeInit; pozwala wielokrotne
  //     wywołania DOMContentLoaded (rzadko używane).
  if (typeof global.vildaOnReady !== 'function') {
    const onReadyRegistry = [];
    let initialFired = false;
    let firstTurboLoadConsumed = false;
    ...
    function runRegisteredFirstTime() {
      ...
      onReadyRegistry.forEach(function (reg) {
        // Pierwsze odpalenie: respektujemy `vildaSafeInit` z `once: true`,
        // żeby zachować backward-compat (te same nazwy nie odpalają się dwukrotnie
        // gdy moduł zarejestruje się dwa razy w trakcie pierwszego renderu).
        if (typeof global.vildaSafeInit === 'function') {
    ...
    function runRegisteredOnTurboNav() {
      // Turbo nawigacja — re-init wszystkich, którzy nie wycofali się przez
      // `turboReload: false`. NIE używamy `vildaSafeInit` (registry "once") —
      // tu chodzi o ponowne związanie z nowym DOM-em.
      onReadyRegistry.forEach(function (reg) {
    ...
    global.vildaOnReady = function (name, fn, options) {
      ...
      // Rejestracja po fazie loading? Odpalamy natychmiast (zachowanie identyczne
      // jak wcześniej w starym vildaOnReady).
    ...
    // Trigger 1: pierwszy render strony (DOMContentLoaded).
    if (global.document) {
      if (global.document.readyState === 'loading') {
        global.document.addEventListener('DOMContentLoaded', runRegisteredFirstTime, { once: true });
      } else {
        // DOM już gotowy — flag tak żeby kolejne push do registry odpalały od razu.
        initialFired = true;
      }

      // Trigger 2: turbo:load (po nawigacji bez reloadu). Pierwszy turbo:load
      // pokrywa się z DOMContentLoaded — pomijamy, żeby uniknąć double-init.
      global.document.addEventListener('turbo:load', function () {
        if (!firstTurboLoadConsumed) {
          firstTurboLoadConsumed = true;
          // Edge case: turbo:load mógł zostać wywołany przed DOMContentLoaded.
          // Wtedy nasze inity jeszcze się nie odpaliły — odpalamy je teraz.
    ...
    // Eksponujemy snapshot rejestru dla debugowania (read-only z punktu widzenia
    // konsumenta — modyfikacja tablicy nie wpływa na działanie).
    global.vildaOnReady._registry = onReadyRegistry;
  }

  // ============ vildaPageReady — dla inline-scriptów w HTML ============
  // Skrót do `vildaOnReady` przeznaczony do hurtowej zamiany inline-scriptów,
  // które wcześniej używały `document.addEventListener('DOMContentLoaded', cb)`.
  // Każde wywołanie dostaje auto-generowaną nazwę i jest re-initowane przy
  // każdej Turbo-nawigacji (DOM ma nowe elementy, więc trzeba ponownie podpiąć
  // listenery do togglerów, klików, etc.).
  if (typeof global.vildaPageReady !== 'function') {
    let pageReadyCounter = 0;
    global.vildaPageReady = function (fn) {
      if (typeof fn !== 'function') return;
      const name = 'inline:auto-' + (++pageReadyCounter);
      if (typeof global.vildaOnReady === 'function') {
        return global.vildaOnReady(name, fn);
      }
      // Fallback gdy vildaOnReady nie jest jeszcze zdefiniowany (raczej niemożliwe,
      // bo siedzi w tym samym IIFE) — zwykły DOMContentLoaded.
  ...
  // Wrapper na setInterval, który rejestruje wszystkie tykające intervale
  // i kasuje je w `turbo:before-cache` (zdarzenie Turbo emituje przed
  // umieszczeniem bieżącej strony w cache podczas nawigacji w przód).
  // Bez tego intervale ustawione w modułach typu `flu_therapy.js` /
  // `obesity_therapy.js` żyją dalej po Turbo-nawigacji i sprawdzają
  // visibility nieistniejących już elementów.
...
(function () {
  ...
  // Delegate-handler na klik w przyciski sidebara — listener jest na
  // `document`, więc działa niezależnie od tego, kiedy sidebar zostaje
  // zamontowany (vilda_chrome.js może go dodawać async po mount-headera).
  // Re-fire vildaOnReady przy każdej Turbo-nawigacji wywoła syncSidebarMenuState,
  // ale NIE doda kolejnego listenera (vildaInstallGlobalListener guard).
  function handleSidebarMenuClick(e) {
    var target = e.target;
    if (!target || typeof target.closest !== 'function') return;

    var sidebarSaveBtn = target.closest('#saveDataBtnSidebar');
    if (sidebarSaveBtn) {
      e.preventDefault();
      var headerSave = document.getElementById('saveDataBtn');
      if (sidebarSaveBtn.hasAttribute('disabled')) {
        var msg =
          (headerSave && (headerSave.getAttribute('data-tip') || headerSave.getAttribute('title'))) ||
          sidebarSaveBtn.getAttribute('data-tip') ||
          'Aby zapisać dane, uzupełnij wymagane pola.';
        if (typeof showTooltip === 'function') {
          showTooltip(sidebarSaveBtn, msg);
        } else {
          alert(msg);
        }
        return;
    ...
    var sidebarLoadBtn = target.closest('#loadDataBtnSidebar');
    if (sidebarLoadBtn) {
      e.preventDefault();
      var headerLoad = document.getElementById('loadDataBtn');
      var sidebarFile = document.getElementById('fileInputSidebar');
      if (sidebarLoadBtn.hasAttribute('disabled')) {
        var msg2 =
          (headerLoad && (headerLoad.getAttribute('data-tip') || headerLoad.getAttribute('title'))) ||
          sidebarLoadBtn.getAttribute('data-tip') ||
          'Wczytywanie danych jest możliwe tylko na początku sesji.';
        if (typeof showTooltip === 'function') {
          showTooltip(sidebarLoadBtn, msg2);
        } else {
          alert(msg2);
        }
        return;
      }
      if (headerLoad && typeof headerLoad.click === 'function') {
        headerLoad.click();
      } else if (sidebarFile && typeof sidebarFile.click === 'function') {
        sidebarFile.click();
      }
      return;
    }
  ...
  function initSidebarMenu() {
    // Per-nav: sync stan disabled przycisków z hamburgerem (musi się odpalić
    // po każdej Turbo-nawigacji, bo nowy DOM mógł zmienić stan formularza).
    syncSidebarMenuState();

    // One-shot: globalne listenery na document (capture). vildaInstallGlobalListener
    // gwarantuje że dokładnie jeden raz w cyklu życia karty.
    if (typeof window.vildaInstallGlobalListener === 'function') {
      window.vildaInstallGlobalListener(
        'custom-fixes:sidebar-menu:click', document, 'click', handleSidebarMenuClick, false);
      window.vildaInstallGlobalListener(
        'custom-fixes:sidebar-menu:input', document, 'input', handleGlobalInputChange, true);
      window.vildaInstallGlobalListener(
        'custom-fixes:sidebar-menu:change', document, 'change', handleGlobalInputChange, true);
    }
  }
...
(function(){
  ...
  function updateMiniSummary() {
    ...
    // Steroid-summary mountuje się TYLKO w decor-sidebarze (≥1400px) — gdy
    // decor zniknie (resize w dół), steroid-summary jest usuwany z DOM przez
    // bindSteroidResizeWatcher. Dawny fallback przepychający go z powrotem do
    // lewego sidebara został wycięty — chcieliśmy „steroid-summary widoczny
    // tylko gdy decor widoczny" (2026-05). Jeśli element zostanie poza
    // decor-sidebarem, używamy window.positionSteroidSummary do korekty.
    try {
      ...
      if (mini && miniStyle === 'none') {
        ...
        if (fnameFix === 'steroidy.html') {
          ...
          if (sterEl && typeof window.positionSteroidSummary === 'function') {
  ...
  function initMiniSummary() {
    ...
    // Listener instalowany RAZ przez vildaInstallGlobalListener — bez tego
    // każda Turbo-nawigacja dodawała kolejnego, multiplikując pracę przy resize.
    if (typeof window.vildaInstallGlobalListener === 'function') {
      window.vildaInstallGlobalListener(
        'custom-fixes:mini-summary:resize',
        window,
        'resize',
        function () {
  ...
  function initSteroidShortcuts() {
    ...
    // Steroid-shortcuts pokazujemy TYLKO w decor-sidebarze (>=1400px). Na
    // węższych ekranach decor jest ukryty — wtedy NIE tworzymy w ogóle
    // kontenera, bo zaśmiecałby lewe menu nawigacyjne. Po rozszerzeniu okna
    // resize-handler ponownie wywoła initSteroidShortcuts.
    if (window.innerWidth < 1400) {
      bindSteroidResizeWatcher();
      return;
    }
    ...
    // Sprawdź czy decor jest faktycznie widoczny (CSS @media może go ukryć
    // mimo szerokiego viewportu — np. gdy media-query nie pasuje). Zabezpiecza
    // przed sytuacją gdy initSteroidShortcuts mountuje do niewidzialnego decora.
    var decorStyle = window.getComputedStyle(decorSidebar);
    if (!decorStyle || decorStyle.display === 'none') {
      bindSteroidResizeWatcher();
      return;
    }
    // Avoid duplicates
    ...
    // Mount BEZPOŚREDNIO w decor-sidebarze (po prawej, ≥1400px). Lewy sidebar
    // celowo pomijamy — steroid-summary tam nie należy (rozsadza menu).
    decorSidebar.appendChild(container);
    ...
    // Expose for legacy IntersectionObserver fallback (lines wcześniej w
    // updateMiniSummary). Funkcja teraz tylko USUWA element gdy decor zniknie
    // — nigdy nie wraca do lewego sidebara.
    window.positionSteroidSummary = function () {
      var cont = document.getElementById('steroidSummary');
      if (!cont) return;
      var decor = document.querySelector('.desktop-layout .decor-sidebar');
      var visible = false;
      if (decor) {
        var st = window.getComputedStyle(decor);
        visible = st && st.display !== 'none';
      }
      if (visible && decor) {
        if (cont.parentElement !== decor) decor.appendChild(cont);
      } else if (cont.parentElement) {
        cont.parentElement.removeChild(cont);
      }
    };
  }

  // Resize-watcher binduje się raz; przy każdym resize sprawdza czy
  // steroidSummary istnieje — jeśli decor zniknął, kasuje element; jeśli decor
  // pojawił się a elementu brak, reinicjalizuje skróty.
  var steroidResizeBound = false;
  function bindSteroidResizeWatcher() {
    if (steroidResizeBound) return;
    steroidResizeBound = true;
    window.addEventListener('resize', function () {
      ...
      if (decor) {
        var st = window.getComputedStyle(decor);
        decorVisible = st && st.display !== 'none';
      }
      var existing = document.getElementById('steroidSummary');
      if (decorVisible && !existing) {
        // Decor pojawił się — odbuduj skróty
        initSteroidShortcuts();
      } else if (!decorVisible && existing) {
        // Decor zniknął — kasuj element z DOM
  ...
  // Bug 2 fix (2026-05): wystawiamy updateMiniSummary globalnie żeby
  // vilda_auth_ui.js mógł je wywołać po reset-app-state (czyszczenie
  // mini-summary po zmianie tożsamości). Plus reagujemy na vilda:app-reset
  // event, który dispatchuje vilda_auth_ui przy każdym resecie sesji.
  window.updateMiniSummary = updateMiniSummary;
  if (typeof window.vildaInstallGlobalListener === 'function') {
    window.vildaInstallGlobalListener(
      'custom-fixes:mini-summary:app-reset',
      document,
      'vilda:app-reset',
      function () {
        try { updateMiniSummary(); } catch (_) { /* noop */ }
      }
    );
  } else {
    document.addEventListener('vilda:app-reset', function () {
      try { updateMiniSummary(); } catch (_) { /* noop */ }
    });
  }
})();
