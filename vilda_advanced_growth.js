...
(function (global) {
  ...
  function syncGrowthDataSourceInputs(options) {
    ...
    try {
      ...
      if (!palRadio || !olafRadio || !whoRadio) {
        return getBmiSourceValue(opts) || 'WHO';
      }
      ...
      if (manualSelection) {
        ...
        } else if (!isGrowthDataSourceAllowed(currentSource, ageYears, proMode, opts)) {
          nextSource = getDefaultGrowthDataSource(ageYears, proMode, opts);
        }
      } else if (!isGrowthDataSourceAllowed(currentSource, ageYears, proMode, opts)) {
        nextSource = getDefaultGrowthDataSource(ageYears, proMode, opts);
      }
      if (!isGrowthDataSourceAllowed(nextSource, ageYears, proMode, opts)) {
        nextSource = getDefaultGrowthDataSource(ageYears, proMode, opts);
      }
  ...
  function updateGrowthDataSourceControls(context, options) {
    const opts = options && typeof options === 'object' ? options : {};
    const age = Number(opts.age != null ? opts.age : getGrowthDataSourceAgeYears(opts)) || 0;
    const state = {
      version: VERSION,
      age,
      action: 'skipped',
      hasAgeSourceInput: hasAgeSourceInput(opts),
