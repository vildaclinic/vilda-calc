...
(function (global) {
  ...
  function showNewTermModal(prefillISO, prefillTime, editObj) {
    ...
    overlay.innerHTML = ''
      ...
      + '<label for="tzNtDurMin">Czas trwania</label>'
      + '<input type="number" id="tzNtDurMin" min="5" max="480" step="5" value="90" inputmode="numeric"> <span id="tzNtDurUnit">min</span>'
    ...
    var durMinEl = overlay.querySelector('#tzNtDurMin');
    var durUnitEl = overlay.querySelector('#tzNtDurUnit');
    ...
    // Dyżur liczymy w GODZINACH (obok jest „Cały dzień"); reszta zajęć w minutach.
    function applyDurUnit() {
      if (selMode === 'activity' && selKind === 'duty') {
        durMinEl.min = '1'; durMinEl.max = '24'; durMinEl.step = '1';
        if (durUnitEl) durUnitEl.textContent = 'godz.';
      } else {
        durMinEl.min = '5'; durMinEl.max = '480'; durMinEl.step = '5';
        if (durUnitEl) durUnitEl.textContent = 'min';
      }
    }
    function updateForm() {
    ...
    for (var kb = 0; kb < kindBtns.length; kb += 1) {
      (function (btn) {
        btn.addEventListener('click', function () {
          ...
          btn.classList.add('is-on');
          if (!editId) {
            if (selKind === 'duty') durMinEl.value = '12';
            else if (ACTIVITY_DEFAULT_MIN[selKind]) durMinEl.value = String(ACTIVITY_DEFAULT_MIN[selKind]);
          }
          updateForm();
        });
      })(kindBtns[kb]);
    ...
    if (editAct) {
      ...
      if (editAct.durationMin > 0) durMinEl.value = String(selKind === 'duty' ? Math.max(1, Math.round(editAct.durationMin / 60)) : editAct.durationMin);
      else if (selKind === 'duty') durMinEl.value = '12';
      else if (ACTIVITY_DEFAULT_MIN[selKind]) durMinEl.value = String(ACTIVITY_DEFAULT_MIN[selKind]);
    } else if (selMode === 'activity' && ACTIVITY_DEFAULT_MIN[selKind]) {
      durMinEl.value = String(ACTIVITY_DEFAULT_MIN[selKind]);
    }
    updateForm();
    ...
    saveBtn.addEventListener('click', function () {
      ...
      if (v.mode === 'activity') {
        ...
        if (!_actAllDay && timeEl && /^([01]\d|2[0-3]):[0-5]\d$/.test(timeEl.value || '')) {
          ...
          var raw = parseInt(durMinEl && durMinEl.value, 10);
          var dm;
          if (selKind === 'duty') {
            if (!isFinite(raw) || raw < 1) raw = 12;
            dm = raw * 60;                 // godziny → minuty
            if (dm > 1440) dm = 1440;      // limit vaulta (24 h)
          } else {
            dm = raw;
            if (!isFinite(dm) || dm < 5) dm = ACTIVITY_DEFAULT_MIN[selKind] || 30;
            if (dm > 480) dm = 480;
          }
      ...
      if (!slotNotes.length) {
        ...
        slotNotes.forEach(function (n) {
          ...
          html += '<div class="tz-hcard' + (done ? ' is-done' : '') + ' ' + (CAT_CLASS[n.category] || '') + '" data-note-id="' + esc(n.id) + '">'
            ...
            + '<div class="tz-hcard__nm">' + esc(n.dueTime) + ' · ' + esc(nmOf(n)) + '</div>'
            + (function () {
                var prt = [];
                if (n.title) prt.push(esc(n.title));
                if (n.durationMin) prt.push(durLabel(n));
  ...
  function dayCardsHtml(lookup) {
    ...
    function wideCard(n) {
      ...
      if (n.durationMin) meta.push(durLabel(n));
      ...
      return '<div class="tz-hcard tz-hcard--wide' + (done ? ' is-done' : '') + ' ' + (CAT_CLASS[n.category] || '') + '" data-note-id="' + esc(n.id) + '">'
        + '<span class="tz-hcard__nm">' + (n.dueTime ? esc(n.dueTime) + ' · ' : '') + esc(nmOf(n)) + '</span>'
