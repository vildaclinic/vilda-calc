...
function initFluTherapyModule() {
  ...
  // interwał.
  // `vildaSetInterval` (wrapper z custom-fixes.js) kasuje aktywne intervale
  // przy nawigacji Turbo, żeby nie tykały dalej po przejściu na inną podstronę.
  const _setInt = (typeof window !== 'undefined' && typeof window.vildaSetInterval === 'function')
    ? window.vildaSetInterval : setInterval;
  const _clearInt = (typeof window !== 'undefined' && typeof window.vildaClearInterval === 'function')
    ? window.vildaClearInterval : clearInterval;
  const intervalId = _setInt(() => {
    showFluBtnIfAbxVisible();
