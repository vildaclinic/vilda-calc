...
(function(){
  ...
  function isProfessionalModeEnabled() {
    ...
    try {
      const stored = localStorage.getItem('resultsMode');
  ...
  async function runCircPdfTask(triggerBtn, taskFn) {
    ...
    try {
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = originalHtml || originalText || 'Generuj siatkę';
      }
    }
  }
  ...
  function handleResultsModeSyncEvent(event) {
    ...
    if (detail && typeof detail.professional === 'boolean') {
      try { window.professionalMode = !!detail.professional; } catch (_) {}
    }
    syncCircChartAccess();
    updateCirc();
  }
