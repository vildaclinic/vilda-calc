...
(function (global) {
  ...
  // Faza 2 / krok 8: counter cichych catch'y (działa nawet gdy logger jest w trybie produkcyjnym).
  // Dostępny przez VildaLogger.getSwallowedCatchCount() i przez window.__vildaSwallowedCatchCount.
  let swallowedCatchCount = 0;

  function logSwallowedCatch(moduleName, err, meta, options) {
    swallowedCatchCount += 1;
    // Fast-path produkcyjny: bez debug nie schodzimy w pełną ścieżkę logującą
    // (warn → log → normalize → dedupe → events → console). Same try/catch'e
    // w app.js wołają tę funkcję ~600 razy podczas typowej sesji — fast-path
    // pozwala uniknąć niepotrzebnych alokacji obiektów meta i string'ów.
    // W razie potrzeby diagnostyki: ?vildaDebug=1 w URL lub localStorage.vildaDebug='1'.
    if (!shouldConsoleLog()) {
      return null;
    }
    try {
  ...
  function getSwallowedCatchCount() {
    return swallowedCatchCount;
  }
