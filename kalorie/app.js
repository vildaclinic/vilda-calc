...
// --- KONIEC: Funkcje Droga do normy BMI ---
  ...
  */
  ...
  function __renderPrevSummary(data){
    ...
    // kg difference
    if(kgText){
      // kgText zawiera liczbe kg i opis, np. "2,3 kg do górnej granicy normy"
      const parts = kgText.replace(/"/g, '').split(' ');
      const kgVal = parts.shift();
      const desc  = parts.join(' ');
      rows.push(`<div class="label">Do normy BMI</div><div class="val"><span class="result-val">${kgVal}</span> <span class="muted">${desc}</span></div>`);
    }
    // Insert into DOM
...
function repositionDoctor() {
  ...
  if (isMobile) {
    ...
    // W widoku szerokim umieść podsumowanie pomiarów pomiędzy sekcją modułu lekarskiego a polem PWZ
    if (prevSummaryWrap && prevSummaryWrap.parentElement !== doctorContainer) {
      if (pwzContainer && pwzContainer.parentElement === doctorContainer) {
        doctorContainer.insertBefore(prevSummaryWrap, pwzContainer);
      } else {
        doctorContainer.appendChild(prevSummaryWrap);
      }
    }
    // Ustaw kolejność: upewnij się, że karta podsumowania jest przed sekcją lekarza w widoku desktop
    if (prevSummaryWrap && doctorWrapper && prevSummaryWrap.parentElement === doctorContainer) {
      doctorContainer.insertBefore(prevSummaryWrap, doctorWrapper);
    }
...
function adjustTestButtonWidths() {
  ...
  const ids = ['toggleGhTests', 'toggleOgttTests', 'toggleActhTests', 'toggleEndoTests', 'toggleIgfTests'];
    ...
    isDoctorCheckbox.addEventListener('change', function(){
      ...
      if(this.checked){
        if(storedPwz){
          ...
          // Pozytywna weryfikacja numeru PWZ – pokaż główne przyciski modułu lekarskiego,
          // a ukryj przyciski poszczególnych testów. Upewnij się, że wszystkie listy testów są zwinięte.
          ...
          // Nie pokazujemy od razu listy testów ani kart – użytkownik sam zdecyduje, które otworzyć.
