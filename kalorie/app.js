...
function updatePlanFromDiet(){
  ...
  const diets = proposeDiets(bmr, pal, sex, age < 18);
  ...
  if(bmiReady){
    ...
    if(age >= CHILD_AGE_MIN && age <= CHILD_AGE_MAX){
      ...
      if (bmiPercentile !== null){
        ...
        if (bmiPercentile < 5)                      bmiCat = 'Niedowaga';
        else if (bmiPercentile < normalHiClass)    bmiCat = 'Prawidłowe';
        else if (bmiPercentile < obesityClass)     bmiCat = 'Nadwaga';
        else                                        bmiCat = 'Otyłość';
      } else {
    ...
    if (age >= CHILD_AGE_MIN && age <= CHILD_AGE_MAX && bmiPercentile !== null) {
      // Przygotuj ostrzeżenia o nadwadze/otyłości. Dla dzieci <3 l.
      // stosujemy progi WHO nawet jeśli użytkownik zaznaczył suwak „Polska”.
...
if (age <= 18) {
  ...
  if (statsW && statsH) {
    ...
    if (!statsW && bmiSource === 'WHO' && age * 12 > 120) {
      ...
      if (statsW && typeof w3 === 'number' && statsW.percentile <= 2) {
    ...
    if (statsH.percentile <= 2 && typeof h3 === 'number') {
...
/* ===== KONIEC KARTY CENTYLI ===== */
    ...
    let bmiLine = `<strong>BMI: <span class="result-val">${bmiText}</span>`;
    // Pokaż percentyl, jeśli dostępny
    if (percText) bmiLine += `${percText}`;
    // Kategoria BMI (Niedowaga/Prawidłowe/Nadwaga/Otyłość) wyświetlamy tylko u osób ≥2 lat
    if (age >= 2) bmiLine += ` (${bmiCat})`;
    bmiLine += `</strong>`;
...
if (coleCardEl && coleInfoEl && coleExplanationEl) {
  ...
  if (age > 0 && age <= CHILD_AGE_MAX && weight > 0 && height > 0) {
    ...
    if (lmsBMI && lmsBMI[1] > 0) {
      ...
      const isBMIOver    = (bmiCat === 'Nadwaga' || bmiCat === 'Otyłość');
      const isColeOver   = (coleCat === 'Nadwaga' || coleCat === 'Otyłość');
...
(function renderWHR(){
  ...
  // Render
  whrInfo.innerHTML = `<strong>${result.header}</strong>`;
  whrInfo.style.display = 'block';
  whrInterpret.innerHTML = result.interp + (result.note ? `<br><em>${result.note}</em>` : '');
  whrInterpret.style.display = 'block';
...
function addAdvMeasurementRow() {
  ...
  row.innerHTML = `
    <label>Wiek:
      <input type="number" class="adv-age" min="0" max="18" step="0.1">
    </label>
    <label>Wzrost (cm):
      <input type="number" class="adv-height" min="40" max="250">
    </label>
    <label>Waga (kg):
      <input type="number" class="adv-weight" min="1" max="200">
    </label>
...
function updateAdvAgeMax() {
  ...
  const inputs = document.querySelectorAll('#advMeasurements .adv-age');
  inputs.forEach(inp => {
    if (!isNaN(ageYears)) {
      inp.max = ageYears;
    }
...
function calculateGrowthAdvanced() {
  ...
  // Odczytaj wprowadzone pomiary
  const measRows = document.querySelectorAll('#advMeasurements .measure-row');
  const measurements = [];
  measRows.forEach(row => {
    const ageInput   = row.querySelector('.adv-age');
    const heightInput= row.querySelector('.adv-height');
    const weightInput= row.querySelector('.adv-weight');
    const aVal = parseFloat(ageInput?.value);
    const hVal = parseFloat(heightInput?.value);
    const wVal = parseFloat(weightInput?.value);
    // Dodaj pomiar tylko jeśli wiek jest poprawny (w przeciwnym razie
    // umieszczenie punktu na osi czasu nie ma sensu). Pozostałe
    // pola mogą być opcjonalne – jeśli wzrost lub waga nie są
    // wprowadzone, przyjmij null.
    if (!isNaN(aVal)) {
      measurements.push({
        ageYears: aVal,
        ageMonths: Math.round(aVal * 12),
...
function intakeAddRow(prefill){
  ...
  row.innerHTML = `
    <label>Wiek (lata)
      <input type="number" step="0.1" min="0" class="intake-ageY">
    </label>
    <label>Wiek (miesiące)
      <input type="number" step="1" min="0" max="11" class="intake-ageM">
    </label>
    <label>Masa (kg)
      <input type="number" step="0.1" min="0" class="intake-wt">
    </label>
    <button type="button" class="icon remove-intake-row" title="Usuń wiersz">×</button>
  `;
  ...
  ['input','change'].forEach(ev=>{
    row.addEventListener(ev, e=>{
      if(e.target.matches('.intake-ageY,.intake-ageM,.intake-wt')) debouncedIntakeCalc();
    });
  });
...
function readIntakeRows(){
  ...
  document.querySelectorAll('#intakeMeasurements .measure-row-intake').forEach(row=>{
    ...
    if(!isNaN(y) && !isNaN(w)){
      const months = Math.round((y*12) + (isNaN(m)?0:m));
      out.push({ ageYears: months/12, months, weight: w });
    }
  });
...
function intakeAutofill(){
  ...
  if(!isNaN(basic.weight)){
    intakeAddRow({ ageMonths: basic.ageMonths, weight: basic.weight });
  }
  ...
  if (basic.ageYears < 18 && window.advancedGrowthData && Array.isArray(window.advancedGrowthData.measurements)){
    window.advancedGrowthData.measurements.forEach(m=>{
      if (m && typeof m.ageMonths==='number' && typeof m.weight==='number'){
        ...
        if(!dupe) intakeAddRow({ ageMonths: m.ageMonths, weight: m.weight });
      }
    });
  }
...
function calcEstimatedIntake(){
  const {sex, height} = getUserBasics();
  ...
  for(let i=0;i<rows.length-1;i++){
    ...
    if(childPair){
      const stats0 = calcPercentileStats(a.weight, sex, a.ageYears, 'WT');
      if(stats0){
        const lms1 = getChildLMS(sex, b.ageYears, 'WT');
        if(lms1){
          expectedGain = lmsToValue(lms1[0], lms1[1], lms1[2], stats0.sd) - a.weight;
    ...
    const intakePerDay      = tee + (dW * KCAL_PER_KG) / days;
  ...
  if(!isMobile){
    ...
    if (intervals.some(r=>r.isChild)) {
      html += `<p class="muted" style="margin-top:.25rem;">* Oczekiwany przyrost liczony przy założeniu utrzymania tego samego <em>z‑score</em> (centyla) masy ciała.</p>`;
    }
    res.innerHTML = html;
    return;
  }
  ...
  if (intervals.some(r=>r.isChild)) {
    cards += `<p class="muted" style="margin:.25rem 0 0;">* Oczekiwany przyrost liczony przy założeniu utrzymania tego samego <em>z‑score</em> (centyla) masy ciała.</p>`;
  }
  res.innerHTML = cards;
}
...
/* Interpretacja WHR – zwraca obiekt do renderu */
function interpretWHR(ageY, sex, waistCm, hipCm, bmiVal, bmiPercentile, coleCat){
  ...
  let header = `WHR: <span class="result-val">${whr}</span>`;
  let interp = '';
  let tableHtml = '';
  let showTable = false;
  ...
  if (ageY >= 18){ // DOROŚLI – WHO
    ...
    // DZIECI – oceniamy centyle TALII/BIODER; WHR podajemy liczbowo
    ...
    if (pc){
      ...
      // kategoryzacja ryzyka otyłości brzusznej wg talii
      let risk = '';
      if (pc.waistP >= 95){
        risk = 'Talia &ge;95. centyla – <strong>istotnie podwyższone ryzyko otyłości brzusznej</strong>.';
      } else if (pc.waistP >= 90){
        risk = 'Talia &ge;90. centyla – <strong>kryterium centralnej otyłości (IDF)</strong>.';
      } else if (pc.waistP >= 85){
        risk = 'Talia &ge;85. centyla – <strong>podwyższony</strong> WHR/ryzyko centralizacji tkanki tłuszczowej.';
      } else {
        risk = 'Proporcje talii do bioder <strong>w normie</strong> dla wieku.';
      }

      interp = `Talia: <strong>${waistTxt}</strong>, Biodra: <strong>${hipTxt}</strong>. ${risk}`;
      showTable = true;
      tableHtml = `
        <table>
  ...
  // Notka edukacyjna przy „BMI w normie, WHR wysoki”
  let note = '';
  if (ageY>=18){
    const lim = ADULT_WHR_LIMIT[sex] || 0.90;
    ...
    const coleOver = (coleCat==='Nadwaga' || coleCat==='Otyłość');
  ...
  return { whr, header, interp, tableHtml, showTable, note };
}
