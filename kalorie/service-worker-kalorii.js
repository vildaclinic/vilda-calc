...
if (typeof workbox !== 'undefined') {
  ...
  // ---------------------------------------------------------------------
  // Two large modules, ds_lms.js (Down syndrome LMS reference data) and
  // gh_igf_therapy.js (growth hormone/IGF‑1 therapy calculator), should
  // always be fetched from the network when online.  Using a NetworkFirst
  // strategy ensures that clinicians see the most recent dosing tables and
  // algorithms while still falling back to a cached copy when offline.  We
  // register this route before the generic asset handler so that it takes
  // precedence.
  workbox.routing.registerRoute(
      ...
      caches.match(event.request).then((cachedResponse) => {
        ...
        return fetch(event.request)
          ...
          .catch(() => {
            ...
            // response is available, return the cached HTML file.  We
            // reference the updated filename "Kalkulator_kalorii_pwa 20.html".
            if (event.request.mode === 'navigate') {
              return caches.match('Kalkulator_kalorii_pwa.html');
            }
          });
      })
    );
